...
<html xmlns="http://www.w3.org/1999/xhtml" lang="GBK">
 ...
 function trAdd(the_tab,type){
    ...
    count=len;  
    if(count==1&&$("xydm").value==""){
       alert("请先选择要操作的<bean:message key="lable.xsgzyxpzxy" />！");
       return false;
    }   
	var cellfu =[
	function(data){
...
function dataSave(the_tab){
   ...
   showTips('处理数据中，请等待......');
   refreshForm('/xgxt/jxgl_jhzy.do?method=jxrcJtAdd&doType=save&xn='+xn+'&xq='+xq+'&nd='+nd);
   $("buttonSave").disabled=true;
}

function onShow(){
	var pk = $("xydmV").value+$("xnV").value+$("xqV").value+$("ndV").value;
	if(pk != ""){
		dwr.engine.setAsync(false);
		jxglJhzyDAO.dao_getJxrcJt(pk,function(data){
...
function refreshXy(){
   ...
   var xq=$("xqV").value;
   var nd=$("ndV").value;
   refreshForm("/xgxt/jxgl_jhzy.do?method=jxrcJtAdd&xn="+xn+"&xq="+xq+"&nd="+nd);
}

	</script>
	<body onload="cheCkSjfw();onShow();xyDisabled('xy');">
		<html:form action="/jxgl_jhzy">	
			...
			<input type="hidden" id="xydmV" name="xydmV" value="${xydmV}"/>	
			<input type="hidden" id="userType" name="userType" value="${userType}"/>				
			<div class="title">
				<div class="title_img" id="title_m">
					当前位置：军训管理 - 军训日程 - 具体安排信息维护
				</div>
			</div>
			<table class="tbstyle" border="0" id="rsTable" align="center"
				style="width: 100%">
                   <tr style="height: 23px">					
					...
					<td align="left">
						<html:select property="xydm" onchange="refreshXy()" styleId="xy">
             ...
             <fieldset>
				<legend>
					<p>
						&nbsp;&nbsp;&nbsp;<font color="blue">军训期间学校整体安排情况</font> 
					</p>
				</legend>
						...
						<td>
							<div class="mid_box">
								<table align="center" style="width: 100%" id="rsT" class="tbstyle">								
									...
									<logic:iterate name="rsZtInfo" id="s">
										<tr >
											 <td nowrap />
											 ...
											 <td nowrap />
                                             <bean:write name="s" property="nr"/>
											</td>
											 <td nowrap />
                                             <bean:write name="s" property="dd"/>
											</td>
											 <td nowrap />
                                             <bean:write name="s" property="zzry"/>
											</td>
											 <td nowrap />
                                             <bean:write name="s" property="rq"/>
											</td>
											 <td nowrap />
                                             <bean:write name="s" property="kssj"/>
											</td>
											<td nowrap />
                                             <bean:write name="s" property="jssj"/>
											</td>
										</tr>
									</logic:iterate>
								</table>
