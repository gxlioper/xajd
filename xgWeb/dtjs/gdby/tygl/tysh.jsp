...
<html>
	...
	function isSh(){
		if(curr_row != null){
			...
			if((userType == "fdy" && xysh == "通过") || (userType == "xy" && xxsh == "通过")){
				...
				modi('gdby_tygl.do?method=tyshone&doType=shone');
			}
		}else{
				alert('请选择要审核的数据行！');
		}
	...
	<body onload="choiceDisabled('xy','xy');">
		<html:form action="/gdby_tygl.do?method=tysh" method="post">
						...
						<tr>
							...
							<td width="10" align="center" valign="middle" rowspan="3">
								...
								<button type="button" class="button2" id="search_go" style="height:20px"
									onclick="allNotEmpThenGo('/xgxt/gdby_tygl.do?method=tysh&go=go')">
									查 询
								</button>
								<br/>
					...
					<TR>
						<TD>
							<jsp:include flush="true"
								page="/sjcz/turnpage.jsp?form=tyglForm"></jsp:include>
						</TD>
					</TR>
					<TR>
						<TD height=3></TD>
			...
			<div class="buttontool" id="btn"
				...
				<logic:equal value="yes" name="writeAble" scope="request">
					<button type="button" class="button2"
						onclick="batchData('primarykey_cbv','gdby_tygl.do?method=tysh&shjg=tg&doType=sh&go=go','sh')"
						style="width:80px">
						通过
					</button>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" class="button2"
						onclick="batchData('primarykey_cbv','gdby_tygl.do?method=tysh&shjg=btg&doType=sh&go=go','sh')"
						style="width:80px">
						不通过
					</button>
					&nbsp;&nbsp;&nbsp;&nbsp;
