...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function searchRs(){
				...
				//按钮控制
				var url = "xljk_hzny_ajax.do?method=xszxSearch";
				...
				xszxSearch(url,parameter);
			
		
		}
		
		function xszxSearch(url,parameter){
		...
		function save(tag){
			if(tag=="ok"){
				...
				var url = "xljk_hzny_ajax.do?method=xszxModi";
		...
		function showModi(){
			...
			if(len==1){	
				...
				var zdbr=jQuery("[name=div_pkValue]:checked").parent().parent().find("td").eq(5).html();
				
				if(zdbr=="否"){
					alertInfo("该记录非针对本人评价，无法填写反馈意见，请确认！");
					return false;
				}else{
				
					var pkValue=jQuery("[name=div_pkValue]:checked").val();
					
					var url="xljk_hzny.do?method=xszxDetail&doType=modi";
		...
		function showView(){
			...
			if(len==1){	
				...
				var url="xljk_hzny.do?method=xszxDetail&&doType=view";
		...
		function deleteYbbx(){
			...
			if(n>0){
				...
				if(blog){
					confirmInfo("该操作将会删除学生咨询信息，是否确定继续操作？",function(tag){
						...
						if(tag=="ok"){
							...
							var url= "xljk_hzny_ajax.do?method=deleteXszx";
	...
	<body onload="onShow()">
		...
		<div class="tab_cur">
			<p class="location">
				<em>您的当前位置：</em><a>心理健康-心理咨询师-学生咨询维护 </a>
			</p>
		</div>
			...
			<div class="toolbox" id="dgncz">
				...
				<div class="buttonbox">
					<ul>
						...
						<li>
							<a href="#" onclick="showTopWin('xljk_hzny.do?method=xszxDetail',800,460);return false;" class="btn_zj">咨询意见</a>
						</li>
						<li>
						...
						<logic:equal name="zxs" value="true">
							<li>
								<a href="#" onclick="showModi();return false;" class="btn_xg">反馈</a>
							</li>
						</logic:equal>
						<li>
