...
<script type="text/javascript">	  
		function tjjg(){
			...
			var xq = val('xq');
			var xydm = val('xy');
			qgzxgzkh.checkExists('view_czxx_xsjxjb','xn||xq||xydm',xn+xq+xydm,function(data){
				if (data != null && data == true) {
					qgzxgzkh.checkExists('pjpy_bmshtjb','zjz||dm||bm||tjzt||tjxm',xn+xq+xydm+'xy1jxj',function(data){
						if(data != null && data == true){
							alert('该<bean:message key="lable.xsgzyxpzxy" />奖学金评审结果已经提交！');
							return false;
						}else{//提交
							qgzxgzkh.checkExists('view_czxx_xsjxjb','xn||xq||xydm||xysh||fdysh',xn+xq+xydm+'未审核通过',function(data){
								if(data){
									alert('该<bean:message key="lable.xsgzyxpzxy" />还有部分奖学金数据未审核,暂时不能提交结果！');
									return false;
								}else{
									if (confirm('确定要提交所有审核的数据吗?')) {
										refreshForm("pjpy_czxx_jxjBmsh.do?act=tj&tjxydm=" + xydm);										
									}
									return false;
								}
							});
					...
					alert("该<bean:message key="lable.xsgzyxpzxy" />尚未申请任何奖学金数据,不能进行提交!");
					return false;
				}
			});
		}
		...
		function datasub(type) {
			refreshForm("pjpy_czxx_jxjBmsh.do?act=save&jg="+type + "&yj="+document.getElementById('yj').value);
		}
	...
	if (flag){
		//检测是否有数据审核通过
		CzxxJxjDao.queryShjg(pk,userType,function (data) {
			if (data) {
						...
						<logic:equal value="xy" name="userType">
							...
							<td align="left">
								...
								<button type="button" class="button2" style="height:40px" id="search_go"
									onclick="refreshForm('pjpy_czxx_jxjBmsh.do?act=qry');">
									查询
								</button>
							</td>
							</tr>
				...
				<fieldset>
					...
					<table width="100%" id="rsTable" class="tbstyle">
						...
						<logic:iterate name="rs" id="s">
							<tr onclick="rowOnClick(this);" style="cursor:hand;" ondblclick="modiAndDel('pjpy_czxx_xyjxjshView.do?act=view&pkValue=','modi','800','550')">
								<td align="center">
	...
	<center>
		<logic:equal value="yes" name="writeAble">
			<div class="buttontool" id="btn"
				...
				&nbsp;&nbsp;&nbsp;
				<button type="button" class="button2" id="btn_dc" onclick="fhshdata('pjpy_czxx_jxjBmsh.do?act=fh')"
					style="width:80px">
					返回重审
				</button>
			</div>
