		...
		function dataSave(){
			...
			if(act=="modi"){
			   if(confirm("提交修改后，该申请需将重新进行审核！\n\n确定要提交吗？")){
			      refreshForm("/xgxt/jhzy_rych.do?method=sjyxbysRychSq&doType=save");			     
			   }
			}else{
			   getSztzData.getInfoEx("jhzy_sjyxbysb","xh||xn",pkV," (fdysh<>'未审核' or xysh<>'未审核' or xxsh<>'未审核')",function(bool){
			       if(bool){
			           alert("该生已申请"+xn+"学年省级优秀毕业生荣誉称号；\n\n并通过相关部门审核或正在审核中！不能再次申请！ ");
			           return false;
			       }else{
			          refreshForm("/xgxt/jhzy_rych.do?method=sjyxbysRychSq&doType=save");			          
			       }
...
<logic:present name="noPass">
	       <p align="center" style="color: red">该生目前不满足<bean:write name="jhzyPjpyForm" property="xn"/>学年"省级优秀毕业生"荣誉称号申请条件！</p>
	    </logic:present>	
		<input type="hidden" id="url" name="url"
			value="/jhzy_rych.do?method=sjyxbysRychSq" />
