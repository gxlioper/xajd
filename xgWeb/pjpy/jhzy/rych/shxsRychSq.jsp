...
<head>
	...
	<script language="javascript">
		function dataSave(){
			...
			if(zysj.length>1000){
			  	alert("主要事迹字数过大，限1000字内!");
				return false;
			}
			if(zxqjhjqk.length>500){
			  	alert("获奖情况字数过大，限500字内!");
				return false;
			}
			if(bz.length>300){
			  	alert("备注字数过大，限300字内!");
			...
			if(act=="modi"){
			   if(confirm("提交修改后，该申请需将重新进行审核！\n\n确定要提交吗？")){
			      refreshForm("/xgxt/jhzy_rych.do?method=shxsRychSq&doType=save");			     
			   }
			}else{
			   getSztzData.getInfoEx("jhzy_shsb","xh||xn",pkV," (fdysh<>'未审核' or xysh<>'未审核' or xxsh<>'未审核')",function(bool){
			       if(bool){
			           alert("该生已申请"+xn+"学年三好学生荣誉称号；\n\n并通过相关部门审核或正在审核中！不能再次申请！ ");
			           return false;
			       }else{
			          refreshForm("/xgxt/jhzy_rych.do?method=shxsRychSq&doType=save");			          
			       }
...
<body>
	<div class="title">
		<div class="title_img" id="title_m">
			当前所在位置：评价评优 - 荣誉称号申请 - 三好学生
		</div>
	</div>		
	<html:form action="/jhzy_rych" method="post">
		<logic:present name="noPass">
	       <p align="center" style="color: red">该生目前不满足<bean:write name="jhzyPjpyForm" property="xn"/>学年"三好学生"荣誉称号申请条件！</p>
	    </logic:present>		
		<input type="hidden" id="url" name="url"
			value="/jhzy_rych.do?method=shxsRychSq" />
				...
				<logic:empty name="act">
					<td width="34%">
						<html:select property="xn" styleId="xn"
							onchange="refreshForm('/xgxt/jhzy_rych.do?method=shxsRychSq')">
