...
var count = 1;
 function trAdd(the_tab,type){
    ...
    function(data){    
    	return "<input type='text' style='width:60px' id='xxsh" + count + "' readonly='true' name='xxsh' value='未审核'>";	 
     }
	];
   ...
   var tabobj = document.getElementById(the_tab);
    ...
    if(rowLen == 0){
       ...
       for(var i=1;i<rowLen;i++){
        	if($("hdnr"+i).value == ""){
				alert("第"+i+"行,活动内容不能为空，请确定");
				return false;
				}
			if($("hdnr"+i).value.length>300){
				alert("第"+i+"行,活动内容字数做多只能是300个，请确定");
				return false;
			}
			if($("xthdrq"+i)){
    			if($("xthdrq"+i).value == ""){
        ...
        //alert(rowLen);
        //return false;
        refreshForm('/xgxt/pjpyszyqwh.do?method=szyc_dshdUpdate&xxk=wthd&doType=update');
