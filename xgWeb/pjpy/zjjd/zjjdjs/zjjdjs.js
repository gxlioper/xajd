...
function zhszcpExp(url) {
	...
	|| document.getElementById('xq').value == '' || document.getElementById('xq').value == null) {
		alert('该次数据导出是按学年，学期，班级为单位进行输出，请选择！');
		return;
	} else {
		document.forms[0].action = url;
...
//重置
function resets() {
	document.getElementById('tzjkbzdj').value == '';
	document.getElementById('jxjdm').selectedIndex = 0;
	document.getElementById('bjpddj').selectedIndex = 0;
	document.getElementById('szxyj').value == '';
	document.getElementById('fdyyj').value == '';
	return;
}
