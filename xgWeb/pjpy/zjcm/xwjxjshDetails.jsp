...
%>
	...
	function savedata() {
		...
		if ('通过'==sh) {
			zjcmJxjDAO.checkXwjxjShxx(pk, function (data){
			if (data == 'false') {
				alert("该奖学金审核通过人数已达到学校设置的获奖名额,不能再审核通过!");
				return false;
			} else {
				refreshForm('pjpy_zjcm_xwjxjshDetails.do?go=save');
			}
		});
		} else {
			refreshForm('pjpy_zjcm_xwjxjshDetails.do?go=save');
	...
	<body>
		...
		<html:form action="/zjcm_xwjxj" method="post">
			<input type="hidden" name="pkValue" id="pkValue" value="${pkValue}"/>
			<input type="hidden" name="msg" id="msg" value="${msg}"/>
			<div class="title">
				<div class="title_img">
					当前所在位置：评奖评优 - 审核 - 校外奖学金审核
				</div>
			</div>
			<fieldset>
				<legend>
				...
				<table width="100%" class="tbstyle" align="center">
					...
					<tr>
						...
						<td align="left" colspan="3">
							<html:textarea property="xxyj" style="width:100%" rows="4" onblur="chLeng(this,250)"/>
						</td>
					</tr>
					<tr bgcolor="EEF4F9" align="center">
						<td colspan="4">
						<logic:notEqual value="view" name="writa">
							<button class="button2" id="btn_save" onclick="savedata()" style="width:80px" >保存</button>
							&nbsp;
						</logic:notEqual>
						<button class="button2" id="btn_save" onclick="window.close();return false;" style="width:80px">关闭</button>
						</td>
					</tr>
				</table>
			</fieldset>
