		...
		<script type="text/javascript">
			...
			function rsszSave(){
				...
				confirmInfo('您确定要保存吗?',function(type){
					if ('ok' == type){
						refreshForm('pjpy_comm_rssz.do?method=saveRssz&fpfs='+fpfs+'&bl='+bl+'&flow=flow');
						alertInfo("保存成功 ！")
					}
				})
			}
			...
			function szfwSearch(){
				...
				if('xy' == szfw){
					url += "?method=rsszForSzfw&szfw=xy&flow=flow" ;
				}else if('zy' == szfw){
					url += "?method=rsszForSzfw&szfw=zy&flow=flow";
				}else if('bj' == szfw){
					url += "?method=rsszForSzfw&szfw=bj&flow=flow";
				}else{
					url += "?method=rsszForSzfw&szfw=nj&flow=flow";
				}
			...
			function saveQdrs(){
				...
				if( qdrs != null && qdrs.length>0){
					confirmInfo('您确定要保存吗?',function(type){
						if ('ok' == type){
							refreshForm('/xgxt/pjpy_comm_rssz.do?method=saveQdrs&flow=flow');
							alertInfo("保存成功!");
						}
					})
				}else {
			...
			function setMrrs(){
				...
				if(cz<=syrs || syrs == -1){				
					batchData('primarykey_cbv','/xgxt/pjpy_comm_rssz.do?method=saveQdrsFromMrrs&flow=flow','tb')
				}else{
					alertInfo('超过该项目人数上限！');
				}
			}
			...
			function choiceKzfw(){
				var sfrssz = $('sfrssz').value;
				
				if(sfrssz=='false'){
					alertInfo("该评奖项目不需要人数设置！", function(){window.close();});
					return false;
				}
			...
			function batchData(checkname,url,type) {
				...
				if (flag){
					...
					//提示确认信息
					confirmInfo(confirmValue,callBack);
					//提示条信息
					if ($("pt")){
			...
			function callBack(tag){
				if(tag=='ok'){
					document.forms[0].action = '/xgxt/pjpy_comm_rssz.do?method=saveQdrsFromMrrs&flow=flow';
	...
	<body onload="choiceKzfw();">
		...
		<div class="flow-steps">
			  <ol class="num5">
			    ...
			    <li class="done" style = "width:19%"><span>2. 项目调整范围设置</span></li>
			    <li class="done"><span>3. 项目条件设置</span></li>
			    <li class="done current-prev" style = "width:19%"><span>4. 项目时间设置</span></li>
			    <li class="current" style = "width:19%"><span>5. 项目人数设置</span></li>
			  </ol>
        </div>
		...
		<html:hidden property="xmdm" styleId="xmdm"/>
			...
			<input type="hidden" id="sfrssz" name="sfrssz" value="${sfrssz }" />
					...
					<div class="toolbox">
						<div class="buttonbox">
							<ul>
								...
								<li>
									<a href="#" class="btn_sz"
										onclick="showTopWin('/xgxt/pjpy_comm_rssz.do?method=rsszCssz',400,200);return false;">计算规则</a>
								</li>
								<li>
								...
								<li>
									<a href="#" class="btn_ccg" onclick="saveQdrs();return false;">保存</a>
								</li>
							</ul>
						</div>
					</div>
				...
				<table>
					<tbody>
						<tr>
							...
							<td>
								<html:select property="xmdm" styleId="xmdm" style="width:180px" disabled="true"
