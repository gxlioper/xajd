...
function chgPage(objTr) {
	document.forms[0].action = "pjpy_xmlg_rychCsblsz.do?act=" + objTr.id;
...
function initData() {
	if (confirm('初始化数据将会清空当前评奖学年下已设置的各荣誉称号比例及各<bean:message key="lable.xsgzyxpzxy" />调整的人数,并重新生成!')) {
		if (confirm('该操作不可逆转,确定要继续吗？')) {
			refreshForm("pjpy_xmlg_initDataByRych.do");
			if ($("pt")) {
...
function modiData() {
	...
	bl = bl.replace(/(^\s*)|(\s*$)/g, "");
	if (bl == null || bl == '') {
		alert("尚未设置荣誉称号比例,不能对比例进行调整!，请先进行该奖项的比例设置操作.");
		return false;
	}

	modiAndDel('pjpy_xmlg_modiRychBlszxx.do?pkValue=','modi',600,450);
}
</script>
<body onload="disBmxx();pageCardOn('rych')">
	<html:form action="/xmlgpjpy.do?method=rychCsblsz" method="post">
				...
				<thead>
					<tr>
						<td align="" nowrap="nowrap">
							...
							&nbsp;&nbsp;
							荣誉称号:<html:select property="dm" styleId="dm">
						...
						<td width="10" rowspan="2" align="center" valign="middle">
							<button type="button" class="button2" style="height:40px" id="search_go"
								onclick="refreshForm('pjpy_xmlg_rychCsblsz.do')">
								查询
							</button>
						</td>
					</tr>
					...
					<center>
						<div class="buttontool" id="btn"
							...
							<button type="button" class="button2" id="btn_xg"
								onclick="showTopWin('pjpy_xmlg_rychBlsz.do',860,600)"
								style="width:80px">
								比例设置
							</button>
						</div>
