	...
	<script language="javascript">
		...
		function isSh(){
			if(curr_row != null){
				...
				if((userType == "fdy" && xysh == "通过") || (userType == "njbzr" && xxsh == "通过")){
					alert("您选择的学生已被上级审核通过，您无权修改");
				}else{
					modi('ghxy_ryjf.do?method=grjfViewAndModi&doType=modi');
				}
			}else{
				alert('请选择要修改的数据行！');
			}
						...
						<tr>
							...
							<td width="10" align="center" valign="middle" rowspan="3">
								...
								<button type="button" class="button2" id="search_go" style="height:20px"
									onclick="allNotEmpThenGo('/xgxt/ghxy_ryjf.do?method=grjfcx&go=go')">
									查 询
								</button>
								<br/>
								<button type="button" class="button2" style="height:20px" id="cz" onclick="czSearchCond('xn-xh-xq-nj1-xm-xy-zy-bj');">
									重 置
								</button>
							</td>
						</tr>
					...
					<table width="99%" id="rsTable" class="tbstyle">
						...
						<logic:iterate name="rs" id="s">
							<tr onclick="rowMoreClick(this,'',event);"
								...
								<logic:iterate id="v" name="s" offset="11" indexId="index">
									<td>
										<input type="hidden" value="${v }" />
										${v }
									</td>
			...
			<div class="buttontool" id="btn"
				...
				<logic:equal value="yes" name="writeAble" scope="request">
					<button type="button" class="button2"
						onclick="location='ghxy_ryjf.do?method=grjfsq';" style="width:80px">
						增加
					</button>
				&nbsp;&nbsp;&nbsp;&nbsp;
				...
				&nbsp;&nbsp;&nbsp;&nbsp;
				<button type="button" class="button2"
						onclick="dataBatch('ghxy_ryjf.do?method=grjfcx&doType=del&go=go')"
						style="width:80px">
						删 除
					</button>
				&nbsp;&nbsp;&nbsp;&nbsp;
				...
				<button type="button" class="button2"
					onclick="wjcfDataExport('ghxy_ryjf.do?method=grjfExp',600,400)"
					style="width:80px">
					导出数据
				</button>
			</div>
