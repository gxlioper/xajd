...
<html xmlns="http://www.w3.org/1999/xhtml">
	 ...
	 */
	function add(){
		showDialog('上报',780,550,'xljk_xlwjyjgl_ylxlxsglwh.do?method=xz');
	}
	...
	function del(){
		...
		if (rows.length == 0){
			...
			for(i = 0 ; i < rows.length; i++){
				...
				if(sfmtdm == 'y'){
					showAlertDivLayer("不能删除已面谈的数据，请确认 ！");
					return false;
				}
			}
	...
	*/
	function tj(){
		...
		if (rows.length == 0){
			...
			showDialog('提交预警库',680,330,'xljk_xlwjyjgl_ylxlxsglwh.do?method=tj&xhs=' + ids.toString() );
		}
	...
	function mtlr(){
		...
		if (rows.length != 1){
			...
			showDialog('面谈录入',780,540,'xljk_xlwjyjgl_ylxlxsglwh.do?method=mtlr&xh=' + rows[0]['xh'] );
		}
	}
