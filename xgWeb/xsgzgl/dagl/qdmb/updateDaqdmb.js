...
function  saveForm(){
	...
	if(jQuery("#daqdmb_mc").val()!=jQuery("#oldDaqdmbmc").val()){
		if(!check("daqdmb_mc")){
	...
	var url = "daqdmb.do?method=updateDaqdmb&type=save&daqdclIds="+daqdclIds;
...
jQuery(function() {
	jQuery("#unselectUl, #selectUl").dragsort({
		...
		dragBetween : true,
		dragEnd : saveOrder,
		placeHolderTemplate : "<li><label class='college_li college_checkbox' style='border:#155FBE 1px dotted;background:#CBE4F8;height:20px;line-height:20px!important;*height:28px;width:80px;padding:3px 0px;text-indent: 15px;'></label></li>"
	});
	
	if(jQuery("#qyzt").val()=="0"){
		jQuery("input[type=radio][name=qyztName][value=0]").attr("checked","checked");
		jQuery("input[type=radio][name=qyztName][value=1]").removeAttr('checked');
	}else{
		jQuery("input[type=radio][name=qyztName][value=1]").attr("checked","checked");
		jQuery("input[type=radio][name=qyztName][value=0]").removeAttr('checked');
	}
});
