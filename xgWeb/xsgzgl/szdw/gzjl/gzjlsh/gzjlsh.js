...
function gzjlsqCk(sqid, zgh) {
	showDialog("审核查询", 750, 550, "gzjlsq.do?method=gzjlsqCk&sqid=" + sqid
			+ "&zgh=" + zgh);
}
...
function gzjlsh() {
	...
	if (shzt == "ysh") {
		...
		showDialog("工作记录审核", 700, 550, url);
	} else {
		showDialog("工作记录批量审核", 500, 250, "gzjlsh.do?method=gzjlPlsh");
	}
}
// 批量审核
...
var DCCLBH = "gzjl_gzjlsh.do";//dcclbh,导出功能编号
...
function khshExportData() {
	...
	var shzt = jQuery("#shzt").val();
	var url = "gzjlsh.do?method=exportData&shzt="+shzt+"&dcclbh=" + DCCLBH;//dcclbh,导出功能编号
