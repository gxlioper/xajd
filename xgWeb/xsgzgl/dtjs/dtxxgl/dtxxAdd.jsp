...
<%--			alert(jQuery('[name=jssj]').size());--%>
			...
			systemFunction.checkExists("xg_dtjs_xsdtxxjlb","xh",jQuery("#xh").val(),function(data){
				if(data){//数据已存在
					...
					refreshForm('dtjs_dtxxgl.do?method=dtxxAdd&doType=save');
				}
			});
		}
		...
		function show(jdsx,jddm){
			...
			for(var i=20;i>jdsx;i--){
				jQuery('#sj_'+i+'>input').val("");
			}
			...
			<tbody>
				<tr >
					...
					<td width="34%">
						...
						<button type="button" class="btn_01" id="" onclick="showDialog('请选择一个学生',680,550,'xsxx_xsgl.do?method=showStudents&goto=dtjs_dtxxgl.do?method=dtxxAdd');return false;">
							选 择
						</button>
					</td>
					<th width="16%">
