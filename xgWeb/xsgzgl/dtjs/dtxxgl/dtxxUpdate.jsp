...
<%--			return false;--%>
		...
		function autoButtonSet(){
			var jdsx = jQuery('#jdsx').val();
			if((parseFloat(jdsx)+1)>=jQuery('[name=jddm]').size()){
				updateFalse("buttonNext");
			}else{
				updateTrue("buttonNext");
			}
			if((parseFloat(jdsx)-1)<=jQuery('#dqjdsx').val()||jQuery('#jdsx').val()<=1){
				updateFalse("buttonUp");
			}else{
				updateTrue("buttonUp");
			}
		}
		function go(fx){
			...
			autoButtonSet();
			if(fx=='next'){
				//alert(jdsx+":"+jQuery('#jdsx').val()+":"+jQuery('[name=jddm]').size());
				if(jQuery('#jdsx').val()!=jQuery('[name=jddm]').size()){
					jdsx++;
					//updateTrue("buttonUp");
				}
				//if(jdsx==jQuery('[name=jddm]').size()){
				//	updateFalse("buttonNext");
				//}
			}else{
				...
				if(jdsx==jQuery('#dqjdsx').val()||jQuery('#jdsx').val()<=1){
					//updateFalse("buttonUp");
				}
			}
			      ...
			      <tr>
			        <td colspan="4"><div class="bz">"<span class="red">*</span>"为必填项</div>
			          <div class="btn">
			          	...
			          	<button type="button"  name="退回" id="buttonUp" onclick="go('back')">退回</button>
			          	<button type="button"  name="提交" id="buttonSave" onclick="saveDtxx();">保存</button>
			            <button type="button"  name="关闭" id="buttonClose" onclick="iFClose();return false;">关闭</button>
			          </div></td>
			      </tr>
			    </tfoot>
		</table>
