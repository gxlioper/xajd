...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		//检查全部地址 到县
		function checkAllAddress(str){
			//alert(str);
			var spt = str.split("-");
			for (var i = 0; i < spt.length; i++) {
				//如果配置了省，继续检查 市 县
				if(spt[i] != null && spt[i] != ""&&spt[i].indexOf("shen")!=-1){			
					var index=spt[i].indexOf("shen");
					var startStr=spt[i].substring(0,index);
					//alert(spt[i]+":"+startStr);
					var shen=jQuery("#"+startStr+"shen").val();
					var shi=jQuery("#"+startStr+"shi").val();
					var xian=jQuery("#"+startStr+"xian").val();
					//alert(shen+":"+shi+":"+xian);
						if(isNull(shen)||isNull(shi)||isNull(xian)){
							return false;
						}
				}
			}
		...
		function saveZxsSq(btzdId){
			...
			var str = "xh-nj-xydm-zydm-bjdm-zzmm-"+btzdId;
			var flag = checkNotNull(str);
			//982 张昌路 对非空的的省 增加必须填写 市 县。
			flag=flag&&checkAllAddress(btzdId);
			if (!flag) {
		...
		function framecon(){
			var iframe = window.parent.document.getElementById("ifm");
			var midframe = window.parent.document.getElementById("midFrame");
			
			var bHeight = jQuery(iframe.contentWindow.document.body).height();
			var dHeight = jQuery("body").height();
			var mHeight = jQuery(midframe.contentWindow.document.documentElement).height();
		...
		jQuery(function(){
			var xsxxxg = jQuery.trim(jQuery("#xsxxxg").val());
			if(xsxxxg != null && xsxxxg != ""){
	...
	<body >
		...
		<html:form action="/xsxx_tygl"  method="post">
			...
			<input type="hidden" name="xsxxxg" id="xsxxxg" value="${xsxxxg }"/>
...
<div class="demo_xxxx" style="margin-top:20px;width:100%;" id="demo_xxxx" >
</logic:equal>
...
<div class="demo_xxxx" style="margin-top:20px;width:100%;" id="demo_xxxx" >
				...
				<table width="100%" border="0" style="margin-bottom: 2px" class="formlist" id="tab_jbxx">
					...
					<tbody id="hi_jbxx">
						...
						<tr>
							...
							<td width="30%">
									<html:text name='rs'  property="zd1" styleClass="text_nor" styleId="zd1"
								  maxlength="20"/>
							</td>
							<th>
								性别
						...
						<tr>
							...
							<td colspan="2">
								<input type="text" id="xymc" onkeydown="return onlyBackSpace(this);" value="${rs.xymc }" />
				...
				<table width="100%" border="0" style="margin-bottom: 2px" class="formlist" id="tab_lxfs">
					...
					<tr>
							...
							<td align="left"  colspan="2">
								<html:text name='rs' property="zd2" styleId="zd2" maxlength="6" onkeyup="value=value.replace(/[^\d|.]/g,'') "/>
							</td>
						</tr>
						<tr>
							<th width="13%">
				...
				<table style="margin-bottom: 2px" width="100%" border="0" class="formlist" id="tab_jtcyxx">
					...
					<tbody id="hi_jtcyxx">
						<tr>
							<th width="13%">
								<div align="center"><span class="red" >*</span>姓名</div>
							</th>
							<th >
								<div align="center"><span class="red" >*</span>称呼</div>
							</th>
							<th>
								<div align="center"><span class="red" >*</span>工作单位及地址</div>
							</th>
							<th>
								<div align="center"><span class="red" >*</span>单位电话</div>
							</th>
							<th>
								<div align="center"><span class="red" >*</span>个人电话</div>
							</th>
						</tr>
						<tr>
							<td align="center">
				...
				<table style="margin-bottom: 2px" width="100%" border="0" class="formlist" id="tab_qtxx">
					...
					<tr>
						...
						<th>
							...
							<td align="left" colspan="4">
								<html:select name='rs' property="zd5" style="width:120px"
									styleId="zd5">
									<html:option value=""></html:option>
									<html:option value="农村">农村</html:option>
									<html:option value="城市">城市</html:option>
									<html:option value="县镇非农">县镇非农</html:option>
								</html:select>
							</td>
							</tr>
							<tr>
