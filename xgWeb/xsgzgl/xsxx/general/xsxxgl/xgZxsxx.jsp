	...
	<head>
					...
					for (var i=0;i<zdpzArr.length;i++) {
						...
						if ('y'==sfzd) {//只读的处理
							...
							if (zdid == 'jg'){
								...
								var hiddenInput = jQuery("<input type='hidden' id='"+jQuery('#'+zdid).attr("name")+"' name='"+jQuery('#'+zdid).attr("name")+"' value='"+jQuery('#'+zdid).val()+"'/>");
		...
		function initStuzd() {
			if ('stu'==jQuery('#userType').val()) {
				var hiddenNjInput = jQuery("<input type='hidden' id='nj' name='nj' value='"+jQuery('#nj').val()+"'/>");
				jQuery('#nj').parent().html("<div style='word-break:break-all;width:97%'>" 
						+ jQuery('#nj').val()+"</div>").append(hiddenNjInput);
...
<div class="demo_xxxx" style="margin-top:20px;width:100%;" id="demo_xxxx" >
				...
				<input type="hidden" name="zydm" id="zydm" value="${rs.zydm}" />
				...
				<table width="100%" border="0" style="margin-bottom: 2px" class="formlist" id="tab_jbxx">
					...
					<tbody id="hi_jbxx">
						...
						<tr>
							...
							<td colspan="4">
								<html:text name="rs" property="ksh" maxlength="20" 
										   styleClass="text_nor" styleId="ksh" ></html:text>
							</td>
						</tr>
						<tr>
							<th>
