	...
	<head>
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				var gridSetting = {
					pager:"pager",
					url:"xsxxDyxjDyzp.do?method=getZpshList",
					colList:[
					   ...
					   {label:'自评等级',name:'djmc',index:'djmc',width:'13%'},
					   {label:'shzt',name:'shzt', index: 'shzt',hidden:true},
					   {label:'评定等级',name:'sjdjmc', index: 'sjdjmc',width:'11%'},
...
<%--					radioselect:true--%>
			...
			function cancelAuding(){
				...
				if (ids.length != 1){
					showAlertDivLayer("请选择一条您要撤消的记录！");
				} else {
					jQuery.post("xsxxDyxjDyzp.do?method=cancelAudit",{id:ids.toString(),gwid:rows[0]["xtgwid"]},function(data){
			...
			// 批量审核保存
			function savePlsh(shzt,shyj,pddjdm){
				var rows = jQuery("#dataTable").getSeletRow();
				var guid = new Array();
				var splcid = new Array();
				var xtgwid = new Array();
				var xh = new Array();
				jQuery.each(rows,function(i,row){
					guid.push(row["id"]);
					splcid.push(row["splcid"]);
					xtgwid.push(row["xtgwid"]);
					xh.push(row["xh"]);
				});
				jQuery.post("xsxxDyxjDyzp.do?method=dyzpPlSh&type=save",
					{
					 sqid:guid,
					 splcids:splcid,
					 xtgwids:xtgwid,
