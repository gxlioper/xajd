...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		jQuery(function(){
			var gridSetting = {
					caption:"申请列表",
					pager:"pager",
					url:"xszz_xszzbjpy_jglrgl.do?method=jglrManage&type=query",
					colList:[      
						   ...
						   {label:'评议意见',name:'ylzd2', index: 'ylzd2',width:'26%',formatter:function(cellValue,rowObject){
							   ...
							   if(rowObject['tjzt'] == '1' || rowObject['shzt'] != '6'){
								    return jQuery("<span title='"+cellValue+"'>"+cellValueTemp+"</span>");
							   		...
							   		return jQuery("<textarea rows='3' style='width:99%;' name='ylzd2' sqid='"+rowObject['guid']+"' xn='"+rowObject['xn']+"' xq='"+rowObject['xq']+"' xh='"+rowObject['xh']+"' xm='"+rowObject['xm']+"' xmdm='"+rowObject['xmdm']+"'>"+cellValue+"</textarea>");
							   }
							 }
							},
		...
		function xhLink(cellValue,rowObject){
			return "<a href='javascript:void(0);' class='name' onclick='xszzbjpyView(\""+rowObject["guid"]+"\",\""+rowObject["xh"]+"\");'>"+cellValue+"</a>";
		}
		function xszzbjpyView(guid,xh){
			showDialog('资助申请查看',780,520,'xszz_sqsh.do?method=viewXmsq&guid='+guid+'&xh='+xh);
		}
		function searchRs(){
		...
		function bcJglr(){
			...
			var url = "xszz_xszzbjpy_jglrgl.do?method=bcJglr";
			jQuery.post(url,
				{
					values:sqrArr.join(","),
					sqids:sqidArr,
	...
	<body>
		<html:form action="/xszz_xszzbjpy_jglrgl">
			<div class="tab_cur">
				<p class="location">
