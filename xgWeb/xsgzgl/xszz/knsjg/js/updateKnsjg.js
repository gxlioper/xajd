function saveForm(){
	...
	if("10742"==jQuery("#xxdm").val()) {
		var url = "xszz_knsjg.do?method=updateKnsjg&type=update&sqlydm="+sqlydmIds;
	}else if("10277"==jQuery("#xxdm").val()){
		var values=pjyydm();
		var url = "xszz_knsjg.do?method=updateKnsjg&type=update&ylzd5="+values;
	}else {
		var url = "xszz_knsjg.do?method=updateKnsjg&type=update";
	}
      ajaxSubFormWithFun("knsjgForm",url,function(data){
    	 if(data["message"]=="保存成功！"){
    		 ...
    		 showAlert("该学生在"+jQuery.trim(xn)+"学年，"+jQuery("#xq option:selected").text()+"学期已经申请过了！");
