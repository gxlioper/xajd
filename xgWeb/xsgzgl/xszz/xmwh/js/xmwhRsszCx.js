...
var gridSetting;
jQuery(function() {
	var url = "xszz_xmwh_rssz.do?method=xmwhRsszCx&type=query";
	url += "&xmdm=" + jQuery("#xmdm").val();
	var rskzfw = jQuery("#rskzfw").val();
	url += "&rskzfw=" + rskzfw;
	url += "&xslb=" + jQuery("#xslb").val();
...
// 分配名额提示信息
function fpmeTip() {
	var url = "xszz_xmwh_rssz.do?method=xmwhRsszYszrs";
	var sTip = "注：";
	jQuery
			.post(url, {
...
function update() {
	...
	var url = "xszz_xmwh_rssz.do?method=xmwhRsszXg&type=save";
...
function saveForm() {
	...
	divLayerClose();
	var url = "xszz_xmwh_rssz.do?method=xmwhRsszBlsz&type=save";
	ajaxSubFormWithFun("form1", url, function(data) {
		showAlert(data.message);
		query();
...
function lnme() {
	...
	var url = 'xszz_xmwh_rssz.do?method=xmwhRsszLnme';
	url += "&xmdm=" + xmdm;
	var title = "历年名额分配情况";
