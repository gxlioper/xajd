...
function onShow() {
	var url = "xszz_xmwh_jdsz.do?method=xmwhJdszSy";
	jQuery.post(url, {
...
//勾选已选中的项目
function initSet(){	
	var url = "xszz_xmwh_jdsz.do?method=xmwhJdszCx&type=query";
	jQuery.post(url, {
		xmdm : jQuery("#xmdm").val()
	}, function(data) {
		for ( var i = 0; i < data.length; i++) {
			var o = data[i];
			jQuery("input:checkbox[name=xmdmView][value="+o.kjddm+"]").attr("checked","checked");
		}
	}, 'json');
}
...
function saveForm() {
	...
	var url = "xszz_xmwh_jdsz.do?method=xmwhJdszXg&type=save";
