...
function onShow(){
	var url = "xszz_xmwh_tjsz.do?method=xmwhTjszSy";
...
function xnSelect(obj,zqlx){
	...
	var xmdm = jQuery("#xmdm").val();
	var url = 'xszz_xmwh_tjsz.do?method=xmwhTjszXn';
	url += "&xmdm=" + xmdm;
	url += "&xnVal=" + xn;
	url += "&zqlx=" + zqlx;
	url += "&tjdm=" + tjdm;
...
function update() {
	...
	var url = "xszz_xmwh_tjsz.do?method=xmwhTjszXg&type=save";
	jQuery.post(url, {
		xmdm:jQuery("#xmdm").val(),
		json:json
	}, function(data) {
...
function del(){
	...
	showConfirmDivLayer("是否确定删除勾选的记录？",{"okFun":function(){
		jQuery("#dataTable input[name=grid_key]:checked").each(function(index){
			...
			var url = "xszz_xmwh_tjsz.do?method=xmwhTjszSc";
			jQuery.post(url, {
				xmdm : jQuery("#xmdm").val(),
				tjdm : tjdm
			}, function(data) {
