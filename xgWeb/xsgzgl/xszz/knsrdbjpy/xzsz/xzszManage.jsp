	...
	<head>
		...
		jQuery(function(){
			var gridSetting = {
					caption:"班级列表",
					pager:"pager",
					url:"xszz_knsrdbjpy_xzszgl.do?method=xzszManage&type=query",
					colList:[      
						   ...
						   {label:'评议小组人数',name:'xzrs', index: 'xzrs',width:'8%',
							formatter:function(cellValue,rowObject){
										...
										html.bind("click",function(){
											...
											var url = "xszz_knsrdbjpy_xzszgl.do?method=updateXzsz&bjdm=" + rowObject["bjdm"] + "&sfksbjpy="+rowObject["sfksbjpy"];
		...
		function add(){
			...
			if (rows.length != 1) {
				...
				var url = "xszz_knsrdbjpy_xzszgl.do?method=addXzsz&bjdm=" + rows[0]["bjdm"];
		...
		function update() {
			...
			if (rows.length != 1) {
				...
				var url = "xszz_knsrdbjpy_xzszgl.do?method=updateXzsz&bjdm=" + rows[0]["bjdm"] + "&sfksbjpy="+rows[0]["sfksbjpy"];
				var title = "修改班级评议小组成员";
				showDialog(title,790,490,url);
			}
		}
		...
		function submitBusi(){
			...
			if (ids.length != 1 ){
				...
				var url = "xszz_knsrdbjpy_xzszgl.do?method=submitXzsz";
