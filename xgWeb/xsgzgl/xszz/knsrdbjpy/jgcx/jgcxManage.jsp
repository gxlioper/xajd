...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		jQuery(function(){
			var gridSetting = {
					caption:"申请列表",
					pager:"pager",
					url:"xszz_knsrdbjpy_jgcxgl.do?method=jgcxManage&type=query&pyyxbl=${jcszModel.pyyxbl }",
					colList:[      
				         ...
				         {label:'学号',name:'xh', index: 'xh',width:'11%',formatter:xhLink},
						   ...
						   {label:'认定周期',name:'knsrdzq', index: 'knsrdzq',width:'10%'},
						   ...
						   {label:'是否困难',name:'sfknmc', index: 'sfknmc',width:'11%'},
						   {label:'困难等级',name:'kndjmc', index: 'kndjmc',width:'11%'},
						   {label:'评定等级',name:'pjdjmc', index: 'pjdjmc',width:'10%'},
						   {label:'审核状态',name:'shztbjpymc', index: 'shztbjpymc',width:'5%'},
						   {label:'评定等级代码',name:'pjdjdm', index: 'pjdjdm',hidden:true},
		...
		function submitBusi(){
			...
			if (rows.length != 1) {
				...
				// 设置评定等级，便于子页面取值
				jQuery("#pjdjmc_hid").val(rows[0]["pjdjmc"]);
				jQuery("#pjdjdm_hid").val(rows[0]["pjdjdm"]);
				...
				var url = "xszz_knsrdbjpy_jgcxgl.do?method=jgcxTj&sqid=" + rows[0]["guid"];
				var title = "提交班级评议结果";
		...
		function jgcxView(){
			...
			if (rows.length != 1) {
				...
				var url = "xszz_knsrdbjpy_jgcxgl.do?method=jgcxView&xn=" + rows[0]["xn"] + "&xq=" + rows[0]["xq"] + "&sqr=" + rows[0]["xh"] + "&shztbjpy=" + rows[0]["shztbjpy"];
				var title = "查看评议详情";
		...
		function cancleRst(){
			...
			if (ids.length != 1 ) {
				...
				showConfirmDivLayer("您确定要撤销选择的记录吗？", {
					"okFun" : function() {
						jQuery.post("xszz_knsrdbjpy_jgcxgl.do?method=jgcxCx", {
							values : rows[0]["xh"],
							sqid : rows[0]["guid"]
						}, function(data) {
		...
		function exportConfig(){
			var DCCLBH='xszz_knsrdbjpy_jgcx.do';
			customExport(DCCLBH, exportData);
		}
		function exportData(){
			var DCCLBH='xszz_knsrdbjpy_jgcx.do';
			setSearchTj();//设置高级查询条件
			var url = "xszz_knsrdbjpy_jgcxgl.do?method=exportData&dcclbh=" + DCCLBH;//dcclbh,导出功能编号
	...
	<body>
		<html:form action="/xszz_knsrdbjpy_jgcxgl">
			...
			<input type="hidden" id="pjdjmc_hid" value=""/>
			<input type="hidden" id="pjdjdm_hid" value=""/>
