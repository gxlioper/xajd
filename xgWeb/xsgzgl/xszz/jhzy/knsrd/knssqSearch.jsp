...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function searchRs(){
			...
			var url = "jhzy_knsrd.do?method=searchKnssq";
			var ie = "ie";
		...
		function showKnssqAdd(){
			var url = "jhzy_knsrd.do?method=knssqInsert";
		...
		function showKnssqEdit(){
			...
			if(len==1){	
				...
				if(flag){
					...
					var url="jhzy_knsrd.do?method=knssqUpdate";
						url+="&pkValue="+pkValue;
						showTopWin(url,800,600);
				}
			}else{	
			...
			if(len!=0){	
				...
				if(flag){
					...
					confirmInfo("请您确认<font color='blue'>是否删除</font>所勾选的申请记录？",function(tag){
						if(tag=="ok"){
							var url = "jhzy_knsrd.do?method=deleteKnsrdSq";
		...
		function showLcgz(){
			...
			if(len==1){	
				...
				//路径
				var url = "jhzy_knsrd.do?method=createLcgzHtml";
				//参数
			 	var parameter = {
					"str_pk":pk
				};
