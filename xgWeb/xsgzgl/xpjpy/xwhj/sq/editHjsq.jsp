					...
					var html = "<option value=''></option>";
					  if(data && data.length > 0){
					  	for(var i = 0; i < data.length; i++){
						  	html += '<option value="'+data[i].jxdjdm+'" ';
							if(data[i].jxdjdm == '${jxdjdm}'){
								html += ' selected="selected" ';
							}
							html += ' >'+data[i].jxdjmc+'</option>';
						}
					  }
					jQuery("#jxdjdm").html(html);
					jQuery("#jxdjdm").change();
				  },'json');
			}
			...
			function on_changeMc(){
				...
				jQuery.post("xpj_hjsq.do?method=getJxmcList",{jxlbdm:jxlbdm,jsfs:jsfs,jxdjdm:jxdjdm},function(data){
					var html = "<option value=''></option>";
					  if(data && data.length > 0){
						  for(var i = 0; i < data.length; i++){
							  	html += '<option value="'+data[i].jxmcdm+'" ';
								if(data[i].jxmcdm == '${jxmcdm}'){
									html += ' selected="selected" ';
								}
								html += ' >'+data[i].jxmcmc+'</option>';
							}
					  }
					jQuery("#jxmcdm").html(html);
					jQuery("#jxmcdm").change();
				  },'json');
			}
						...
						<td colspan="4">
							...
							<div class="btn">
								<button type="button" onclick="save('update');">
									...
									<button type="button" onclick="save('updatesubmit');">
										提交申请
									</button>
