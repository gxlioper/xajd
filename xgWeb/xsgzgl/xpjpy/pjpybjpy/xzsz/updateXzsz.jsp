	...
	<head>
		...
		jQuery(function(){
			var gridSetting = {
					caption:"学生列表",
					pager:"pager",
					url:"xpjpy_pjpybjpy_xzszgl.do?method=updateXzsz&type=query&bjdm=${rs.bjdm}",
					colList:[      
		...
		function xzszDel() {
			...
			if (ids.length == 0) {
				...
				showConfirmDivLayer("您确定要删除选择的学生吗？",{"okFun":function(){
					jQuery.post("xpjpy_pjpybjpy_xzszgl.do?method=xzszDel",{values:ids.toString(), bjdm: "${rs.bjdm}"},function(data){
		...
		function xsdbBc() {
			...
			if (ids.length != 1) {
				...
				jQuery.post("xpjpy_pjpybjpy_xzszgl.do?method=xsdbBc",{values:ids.toString(), bjdm: "${rs.bjdm}"},function(data){
					showAlert(data["message"],{},{"clkFun":function(){
						if (parent.window){
							refershParent();
						}
