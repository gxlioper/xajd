...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		jQuery(document).ready(function () {
			...
			jQuery.getJSON("xpj_sqsh.do?method=getCheckProgress",{bjdms:bjdms},function(data){
		...
		function getAllZcpmInfo(obj){
			...
			var url = "xpj_sqsh.do?method=viewAllZcpm&xh="+xh;
			var title = "综测排名汇总";
			showDialog(title,800,400,url)
		}
		function saveDgxmSq(obj){
			var url = "xpj_sqsh.do?method=saveJxsq&type=submit&flag=dgtj";
		...
		function checkConditions(){
			var bjdms = jQuery("#bjdms").val();
			jQuery.post("xpj_sqsh.do?method=checkConditions",{bjdms:bjdms},function(data){
				loading("0%");
				interVal = setInterval(getCheckProgress, 1000);
			});
		}
...
// 						}
					...
					<tr>
						<td>
							<a onclick="showDialog('选择申请奖项',500,350,'xpj_sqsh.do?method=plSelectPjxm&xh=<%=e.get("xh") %>');;" 
