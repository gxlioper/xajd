...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			function toBefore(bbdm){
				...
				api.reload(window,"xpj_bbwh.do?method=showYlbg&bbdm="+bbid+"&xmdm="+xmdm);
			}
			...
			function updateXmbb(){
				...
				jQuery.post("xpj_bbwh.do?method=updateXmbb",{bbdm:bbdm,xmdm:xmdm,bblx:bblx},function(data){
					if (Boolean(data)){
			...
			function qx(){
				var xmdm = jQuery("#xmdm").val();
				var bblx = jQuery("#bblx").val();
				jQuery.post("xpj_bbwh.do?method=updateXmbb",{bbdm:"",xmdm:xmdm,bblx:bblx},function(data){
					if (Boolean(data)){
						showAlert("设置成功！",{},{"clkFun":function(){
							jQuery("#cz").removeAttr("onclick");
							jQuery("#cz").unbind("click");
	...
	<body>
		<input type="hidden" id="bbdm" value="${bbwhModel.bbdm }"/>
		<input type="hidden" id="xmdm" value="${bbwhModel.xmdm }"/>
		<input type="hidden" id="bblx" value="${bbxxList[0].bblx }"/>
	
		<div class="djbx">
			<div class="djbx_left">
				<div class="djbx_left1">
					<a href="javascript:toBefore('${bbwhModel.bbdm }');" id="prev" title="上一张表格"></a>
				</div>
				<div class="djbx_left2">
					<div class="djbx_left2_zz" style="display: block">
						<img src="<%=stylePath%>images/djbx.gif" id="bgImg"
							<logic:notEqual value="${bbwhModel.bbdm }" name="xmxx" property="dybb">
							style="display:none;"
							</logic:notEqual>
						/>
					</div>
				...
				<div class="djbx_left3">
					<a href="javascript:toNext('${bbwhModel.bbdm }');" id="next" title="下一张表格"></a>
				</div>
			</div>
...
<%--				</div>--%>
				<div class="djbx_page">
					...
					<div class="dy">
					<logic:equal  name="bbwh" property="bblx" value="2">
						<logic:equal value="${bbwhModel.bbdm }" name="xmxx" property="dysbbb">
							<a id="cz" href="#" class="nocur" onclick="qx();"></a>
						</logic:equal>
						<logic:notEqual value="${bbwhModel.bbdm }" name="xmxx" property="dysbbb">
							<a id="cz" href="#" class="cur" onclick="updateXmbb();"></a>
						</logic:notEqual>
					</logic:equal>
					<logic:notEqual  name="bbwh" property="bblx" value="2">
						<logic:equal value="${bbwhModel.bbdm }" name="xmxx" property="dybb">
							<a id="cz" href="#" class="nocur" onclick="qx();"></a>
						</logic:equal>
						<logic:notEqual value="${bbwhModel.bbdm }" name="xmxx" property="dybb">
							<a id="cz" href="#" class="cur" onclick="updateXmbb();"></a>
