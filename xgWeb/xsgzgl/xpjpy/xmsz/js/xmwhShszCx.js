...
function onShow() {
	var url = "xpj_xmwh_tzsz.do?method=xmwhShszSy";
	jQuery.post(url, {
...
//勾选已选中的项目
function initSet(){	
	var url = "xpj_xmwh_tzsz.do?method=xmwhShszCx&type=query";
	jQuery.post(url, {
		xmdm : jQuery("#xmdm").val()
	}, function(data) {
		for ( var i = 0; i < data.length; i++) {
			var o = data[i];
			jQuery("input:checkbox[name=xmdmView][value="+o.tzjxdm+"]").attr("checked","checked");
		}
	}, 'json');
}
/*保存*/
function saveForm() {
	...
	var url = "xpj_xmwh_tzsz.do?method=xmwhShszXg&type=save";
