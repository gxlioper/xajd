...
function onShow() {
	var url = "xpj_xmwh_tjsz.do?method=xmwhTjszSy";
		...
		for ( var j = 0; j < tjgxList.length; j++) {
			...
			if (o.tjdm == tjgxObj.tjdm && o.gxdm == tjgxObj.gxdm) {
				...
				if (tjgxObj.gxlx == '1' || tjgxObj.gxlx == '2'
						|| tjgxObj.gxlx == '4') {// 数字文本框，下拉框，小数文本框
...
function tjSet(tjObj) {
	...
	// 依赖周期联动
	jQuery(tjObj).parent().parent().find("td[name=ylzqTd]")
			.html(createXn(zqlx));
...
function tjzSelect(obj, gxz) {
	...
	var url = 'xpj_xmwh_tjsz.do?method=xmwhTjszTjzDiv';
	url += "&xmdm=" + xmdm;
	url += "&tjzVal=" + tjz;
	var title = "条件值选择";
...
function xnSelect(obj, zqlx) {
	...
	var xmdm = jQuery("#xmdm").val();
	var url = 'xpj_xmwh_tjsz.do?method=xmwhTjszXn';
	url += "&xmdm=" + xmdm;
	url += "&xnVal=" + xn;
	url += "&zqlx=" + zqlx;
	url += "&tjdm=" + tjdm;
...
function update() {
	...
	var url = "xpj_xmwh_tjsz.do?method=xmwhTjszXg&type=save";
	jQuery.post(url, {
		xmdm : jQuery("#xmdm").val(),
		json : json
	}, function(data) {
		if (data["success"] == false) {
			...
			if(data["bfhlist"]){
				showConfirmDivLayer("保存成功，是否需要查看不符合条件学生？",  {
					"okFun" : function() {
						showDialog("不符合条件学生", 700, 540, "xpj_xmwh_tjsz.do?method=getbfhList&xmdm="+jQuery("#xmdm").val()+"&xmmc="+jQuery("#xmmc").val());
					}
				});
			...
			jQuery("#dataTable input[name=grid_key]:checked").each(
					function(index) {
						...
						var tjdm = jQuery(this).parent().parent().find(
								"[name=tjdm]").val();
						var url = "xpj_xmwh_tjsz.do?method=xmwhTjszSc";
						jQuery.post(url, {
							xmdm : jQuery("#xmdm").val(),
							tjdm : tjdm
						}, function(data) {
