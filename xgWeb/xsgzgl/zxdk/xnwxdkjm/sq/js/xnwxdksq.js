...
function add() {
	...
	if(sfbys == "否" && jQuery.trim(usertype) == "stu"){
		showAlertDivLayer("您不是毕业生，不能进行无息减免申请！");
		return false;
	}
	if( "1"  == cfbz){
		showAlertDivLayer("您已有申请记录，请勿重复填写！");
		return false;
	}
	if( "0"  == jQuery("#dkjl").val() && jQuery.trim(usertype) == "stu"){
		showAlertDivLayer("您无借款记录，不能进行无息减免申请！");
		return false;
	}
	var url = "xnwxdkjm_sq.do?method=add";
	var title = "校内无息借款减免申请增加";
...
function DksqView(sqbh, xh) {
	showDialog("无息借款减免申请查看", 770, 450, "xnwxdkjm_sq.do?method=DksqView&sqid="
			+ sqbh + "&xh=" + xh);
}
...
function xyzsjgExportData() {
	...
	var url = "xnwxdkjm_sq.do?method=exportData&dcclbh=" + DCCLBH;//dcclbh,导出功能编号
...
function lcgz() {
	...
	if (1 != rows.length) {
		...
		showDialog("校内无息借款减免审批流程跟踪",530,310, 'comm_spl.do?method=lcgz&sqid='
