...
function sbsh() {
	...
	if (shzt == "ysh") {
		...
		var url = "ypzl_sh.do?method=sbDgsh&sqid=" + rows[0]["sqid"] + '&shid=' + rows[0]["shid"];
		showDialog("审核", 700, 480, url);
	} else {
		showDialog("批量审核", 500, 250, "ypzl_sh.do?method=sbPlsh");
	}
}
// 批量审核
function savePlsh(shzt, shyj,zd1,zd3) {
	var rows = jQuery("#dataTable").getSeletRow();
	var guid = new Array();
	var gwid = new Array();
	...
	jQuery.post("ypzl_sh.do?method=sbPlsh&type=save", {
		shzt : shzt,
		id : guid,
		gwids : gwid,
		shyj:shyj,
...
function cancelSh(){
	...
	if (rows.length != 1){
		...
		showConfirmDivLayer("您确定要撤消对该记录的审核操作吗？",{"okFun":function(){
			jQuery.post("ypzl_sh.do?method=cxshnew",{shlc:splc,shid:shid},function(data){
				// 判断是否最后一级撤销(1:最后一级撤销成功）
				if("1" == data["cancelFlg"]){
					jQuery.post("ypzl_sh.do?method=cancelSh",{sqid:sqid,shzt:shzt},function(result){
