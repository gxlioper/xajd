	...
	<head>
		...
		function stzz(){
			...
			if (rows.length != 1) {
				...
				if(rows[0]["stzt"]!="预备期"){
					showAlertDivLayer("只有预备期和正式社团才能进行职务结构管理！");
				}else{
					var url = 'ttgl_stjggl.do?method=stjggl&jgid='+ rows[0]["jgid"];
					showDialog("职务结构管理", 790,height, url);
				}
			}
		}
		function ryManage() {
			...
			if (rows.length != 1) {
				...
				if(rows[0]["stzt"]=="预备期"||rows[0]["stzt"]=="正式"){
					...
					showDialog("成员职务管理", 790,height, url);
				}else{
					showAlertDivLayer("只有预备期和正式社团才能进行成员职务管理！");
				}
			}
		}
		...
		<%@ include file="/comm/hiddenValue.jsp"%>
			<div class="toolbox">
				...
				<div class="buttonbox">
					<ul>
						<li>
							<a href="javascript:void(0);" class="btn_zj" onclick="stzz();return false;" >职务结构管理</a>
						</li>
						<li>
							<a href="javascript:void(0);" onclick="ryManage();return false;" class="btn_xg" >成员职务管理</a>
						</li>
					</ul>
				</div>
