function selectBj(){
    var url = "sxzzjy_bjxfjsjg.do?method=selectBj";
    var title = "选择班级";
...
function saveBj(){
    ...
    if (rows.length != 1) {
        ...
        jQuery.post("sxzzjy_bjxfjsjg.do?method=getBjInfo", {bjdm: o["bjdm"]}, function (dataList) {
            var data = dataList[0];
            var dybl = 0;
            if(data.dyrs > 0 && data.bjzrs  > 0 )
...
function save(type){
    ...
    var url = "sxzzjy_bjxfjsjg.do?method=bjxfjsjgSave&type="+type;
