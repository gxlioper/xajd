function add(hblx){
    ...
    var url = "sxzzjy_grxfjshb.do?method=grxfjshbAdd&hblx="+hblx+
        "&jgid="+rows[0]["jgid"]+"&xh="+rows[0]["xh"];
    var title = "申请";
...
function update(hblx) {
    ...
    var url = 'sxzzjy_grxfjshb.do?method=grxfjshbUpdate&sqid=' + sqid+"&xh="+rows[0]["xh"];
    var title = "修改";
    showDialog(title,700,550,url);
...
function selectHblx(type) {
    ...
    var url = 'sxzzjy_grxfjshb.do?method=selectHblx&type='+type+'&values='+ids.toString() ;
    var title = "";
...
function del(hblx) {
    ...
    showConfirmDivLayer("您确定要删除选择的记录吗？", {
        "okFun" : function() {
            jQuery.post("sxzzjy_grxfjshb.do?method=grxfjshbDel", { values : ids.toString(),hblxs:hblx },
                function(data) {
...
function view(jgid,xh) {
    showDialog("查看", 700,550, "sxzzjy_grxfjshb.do?method=grxfjshbView&jgid=" + jgid+"&xh=" + xh);
}

function xhLink(cellValue, rowObject) {
    return "<a href='javascript:void(0);' class='name' onclick='view(\"" + rowObject["jgid"] + "\",\""+rowObject["xh"]+"\");'>" + cellValue
        + "</a>";
}
...
function submit(hblx){
    ...
    var url = "sxzzjy_grxfjshb.do?method=grxfjshbSubmit";
...
function cancel(hblx){
    ...
    showConfirmDivLayer("您确定要撤销选择的记录吗？",{"okFun":function(){
        jQuery.post("sxzzjy_grxfjshb.do?method=grxfjshbCancel",
            {
...
function grxfjshbLcinfo(){
    ...
    if (ids.length != 1){
        ...
        showDialog("审批流程跟踪",580,480,'comm_spl.do?method=lcgz_hb&sqids='
...
function printWord(hblx){
    ...
    if (len == 1) {
        ...
        url = "sxzzjy_grxfjshb.do?method=getPrint&hblx="+hblx;
        ...
        url = "sxzzjy_grxfjshb.do?method=getPrintZip&hblx="+hblx;
        url += "&value=" + ids;
        window.open(url);
    }
}
