...
function getStuInfo(){
	...
	var url="bzjl_bzjg.do?method=addPjxmjg";
...
function saveForm(){
	...
	if (jQuery("#sqly").val().length > 500){
	 ...
	 var url = "bzjl_bzjg.do?method=addBzjljg&type=save";
...
//增加
function add(){
	var url = "bzjl_bzjg.do?method=addBzjljg";
	var title = "增加表彰奖励结果信息";
...
function update(){
	...
	if (rows.length != 1){
		...
		var url = 'bzjl_bzjg.do?method=updateBzjljg&id='+rows[0]["id"]+'&xh='+rows[0]["xh"];
		var title = "修改表彰奖励信息";
		showDialog(title,800,540,url);
	}
}
...
function del(){
	...
	if (ids.length == 0){
		...
		showConfirmDivLayer("您确定要删除选择的记录吗？",{"okFun":function(){
			jQuery.post("bzjl_bzjg.do?method=delBzjljg",{values:ids.toString()},function(data){
...
function exportData(){
	...
	var url = "bzjl_bzjg.do?method=exportData&dcclbh=" + DCCLBH;//dcclbh,导出功能编号
...
function knsView(id,xh){
	showDialog("表彰奖励结果查询",800,473,"bzjl_bzjg.do?method=bzjljgView&id="+id+"&xh="+xh);
}
