...
function viewJxsh(){
	...
	if (rows.length ==0&&"10335"!=xxdm){
		...
		showDialog("奖项审核",700,500,"bzjl_sqsh.do?method=viewJxsh&sqid="+sqid+"&shid="+shid+"&gwid="+gwid);
	}else {
		showDialog("批量审核",500,300,"bzjl_sqsh.do?method=toPlsh");
	}
}
...
//				jQuery.post("comm_spl.do?method=cxshnew",{shlc:splc,shid:shid},function(data){
				jQuery.post("bzjl_sqsh.do?method=cxshnew",{splc:splc,shid:shid,xmdm:xmdm,xh:xh},function(data){
					
						// 判断是否最后一级撤销(1:最后一级撤销成功）
						if("1" == data["cancelFlg"]){
							jQuery.post("bzjl_sqsh.do?method=cancel",{splcid:splc,shid:shid,xmdm:xmdm},function(result){
...
function showSeletTjdw(){
	showDialog("选择统计方式",300,150,"bzjl_sqsh.do?method=selectTjdw",{max:false,min:false});
}
...
function showAudingList(){
	...
	if (rows.length != 1){
		...
		showDialog("奖项审核",750,500,"bzjl_sqsh.do?method=viewJxshList&tjdw="+tjdw+"&shid="+shid+"&bmdm="+id);
	}
}
 ...
 */
function viewShqk(){
	showDialog("评奖评优审核统计",500,400,"bzjl_sqsh.do?method=pjxmShtj");
}
...
function exportData(){
	...
	var url = "bzjl_sqsh.do?method=exportData&shzt="+shzt+"&dcclbh=" + DCCLBH;//dcclbh,导出功能编号
...
function saveZdPlsh(shzt,shyj,xyXmdm,jsonStr,id){
	...
	jQuery.post(
		"bzjl_sqsh.do?method=saveZdPlsh",
		map,function(data){
			showAlertDivLayer(data["message"],{},{"clkFun":function(){
				jQuery("#dataTable").reloadGrid();
			}});
 ...
 */
function viewZdtj(){
	showDialog("评奖评优审核统计",900,400,"bzjl_sqsh.do?method=viewZdtj");
}
...
function fjdc(){
	...
	jQuery.post("bzjl_sqsh.do?method=checkFjExist", map, function(data) {
		if(data == true){								
			setSearchTj();//设置高级查询条件
			var url = "bzjl_sqsh.do?method=fjdc&shzt="+jQuery("#shzt").val();
			url = addSuperSearchParams(url);//设置高级查询参数
			jQuery("form").eq(0).attr("action", url);
			jQuery("form").eq(0).submit();
		}else{
