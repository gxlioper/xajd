...
function xgSqb(){
	...
	if (rows.length != 1){
		...
		showDialog("修改申请表",800,450,"bzjl_sqsh.do?method=updateSqb&sqid="+rows[0]["sqid"]);
	}
}
 ...
 */
function ckSqb(id){
	showDialog("查看申请表",800,495,"bzjl_sqsh.do?method=viewSqb&sqid="+id);
}
 ...
 */
function editSqb(){
	showDialog("填写申请表",800,455,"bzjl_sqsh.do?method=editSqb");
}
...
function qxsq(){
	...
	if (ids.length == 0){
		...
		showConfirmDivLayer("您确定要删除选择的记录吗？",{"okFun":function(){
			jQuery.post("bzjl_sqsh.do?method=cancelXmsq",{values:ids.toString()},function(data){
...
function saveJxsq(type){
	...
	if (jQuery("#sqjx tr").size() == 0){
		showAlert("请选择您要申请的奖项",{},{"clkFun":function(){
			showDialog("选择申请奖项",450,350,"bzjl_sqsh.do?method=selectPjxm&xh="+xh);
		}});
		return false;
	}
	
	var url = "bzjl_sqsh.do?method=saveJxsq&type="+type;
...
//	var parentTxsmDiv = jQuery(api.get('parentDialog').document).find("#txsmDiv")
	...
	if(jQuery("#xxdm").val() == "10355"){
	  ...
	  if(pjxm.size() == 1){
		  var url = "bzjl_sqsh.do?method=checkHjsqCanAdd";
		  var checkResult = "";
			jQuery.ajax({
			type:'post',
			url:url,
...
function exportData(){
	...
	var url = "bzjl_sqsh.do?method=sqExportData&dcclbh=" + DCCLBH;//dcclbh,导出功能编号
...
function submitBusi(){
	...
	if (ids.length == 0){
		...
		showConfirmDivLayer("您确定要提交选择的记录吗？",{"okFun":function(){
			...
			jQuery.post("bzjl_sqsh.do?method=saveUpdateSqbPl",{values:ids.toString()},function(data){
				tips.close();
	...
	if (ids.length == 0) {
		...
		showConfirmDivLayer("您确定要撤销选择的记录吗？", {
			"okFun" : function() {
				...
				jQuery.post("bzjl_sqsh.do?method=cancle", {
					values : ids.toString(),
					lcid : rows[0]['splc']
				}, function(data) {
...
function exceptionDataResolve(){
    ...
    showConfirmDivLayer("您确定要进行提交异常数据处理吗？",{"okFun":function(){
        jQuery.post("bzjl_sqsh.do?method=exceptionDataResolve",function(data){
            showAlertDivLayer(data["message"]);
        },'json');
    }});
}