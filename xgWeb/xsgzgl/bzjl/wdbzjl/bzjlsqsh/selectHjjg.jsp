	...
	<head>
		...
		<script type="text/javascript">
			var gridSetting = {
				caption : "",
				pager : "pager",
				url : "bzjl_sqsh.do?method=searchHjjg",
				colList : [ {
					label : 'key',
					name : 'id',
					index : 'id',
		...
		function add(){
			...
			jQuery.post("bzjl_sqsh.do?method=saveHjxx", {
				xh : xh,
				ids : ids,
				sqid : sqid
			}, function(data) {
		...
		function del() {
			...
			if (ids.length == 0){
				...
				showConfirmDivLayer("您确定要删除选择的记录吗？",{"okFun":function(){
					jQuery.post("bzjl_sqsh.do?method=delHjxx",{values:ids.toString(),xh:xh},function(data){
						rs = data;
						var parentTbody = jQuery(api.get('parentDialog').document).find("#hjxx");
						var rsList = rs["rsList"];
						for(var i = 0;i < rsList.length;i++){
