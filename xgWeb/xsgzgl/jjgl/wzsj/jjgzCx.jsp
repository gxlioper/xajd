...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			var gridSetting = {
					...
					radioselect:false,
					rowNum:20,
					url:"jjgl_jjgzgl.do?method=queryJJgzList&type=1",
					colList:[
					   {name:'sid', index: 'sid',key:true,hidden:true},
					   {label:'规则名称',name:'gzmc', index: 'gzmc'},
					   {label:'录入时间',name:'jlsj', index: 'jlsj'},
			...
			function selectTab(obj,query_type){
				gridSetting['url'] =  "jjgl_jjgzgl.do?method=queryJJgzList&type=" + query_type;
			...
			function xg(){
				...
				if (rows.length != 1){
					...
					var url = "jjgl_jjgzgl.do?method=jjgzXg&sid="+jQuery("#dataTable").getSeletIds()[0];
					var title = "修改";
			...
			**/
			function ck(){
				...
				if (rows.length != 1){
					...
					var url = "jjgl_jjgzgl.do?method=jjgzCk&sid="+jQuery("#dataTable").getSeletIds()[0];
					var title = "查看家教案例详情";
					showDialog(title,780,400,url);
				}
			}
			 ...
			 */
			function xz(){
				showDialog('新增家教案例',780,400,'jjgl_jjgzgl.do?method=jjgzXz');
			}
			...
			function sc(){
				...
				if (rows.length == 0){
					...
					showConfirmDivLayer("您确定要删除所选记录？",{"okFun":function(){
						jQuery.post("jjgl_jjgzgl.do?method=jjgzSc",{sid:ids.toString()},function(data){
	...
	<body>
		...
		<div class="toolbox">
			<!-- 过滤条件 -->
			<div class="searchtab">
				<html:form action="/jjgl_jjgzgl" method="post" >
					<div class="buttonbox">
						<ul>
