...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function searchRs(){
			var url = "qgzx_cjgl_ajax.do?method=cjxxCx";
			var ie = "ie";
			var otherValue = [ie];
		...
		function showModi(type){
			...
			if("update"==type){
				...
				var url="qgzx_cjgl.do?method=cjxxFf";
				if(len==1){
					var tjzt = jQuery("[name=div_pkValue]:checked").parents("tr").find("td").eq(8).text();
					if("已提交"==tjzt && "true"!=isGly && xxdm != "10277"){
						alertInfo("该勾选的记录已提交，不能再进行修改！",function(tag){
							if(tag=="ok"){
								return false;
							}
						});
				...
				if(len==1){
					var pkValue=jQuery("[name=div_pkValue]:checked").val();
					var url="qgzx_cjgl.do?method=cjxxCk";
					url+="&pkValue="+pkValue;
		...
		function tjcx() {
			...
			var url = "qgzx_cjgl_ajax.do?method=cxtj";
			...
			if(len>=1){
				confirmInfo("是否确定"+message+"勾选的记录？",function(tag){
					if(tag=="ok"){
						...
						var parameter={};
						var url="qgzx_cjgl_ajax.do?method=czCjffxx";
						url+="&doType="+type;
						parameter["pkValue"]=str;							
						jQuery.ajaxSetup({async:false});	
						jQuery.post("qgzx_cjgl_ajax.do?method=isHaveFfxx",
								parameter,
								function(result){
		...
		function aotuChecksfPass(){
				jQuery.ajaxSetup({async:false});	
				jQuery.post("qgzx_cjgl_ajax.do?method=aotoTjCjffxx",{},'');
				jQuery.ajaxSetup({async:true});
		}
		...
		<html:form action="/qgzx_cjgl" method="post">
			...
			<div class="main_box">
				...
				<jsp:include flush="true"
					page="/sjcz/turnpage.jsp?form=qgzxCjglForm"></jsp:include>
			</div>
