...
function kysh() {
	...
	if (shzt == "ysh") {
		...
		var url = "qgzx_kycxkyxmsh.do?method=kyDgsh&xmid=" + rows[0]["xmid"] + '&shid=' + rows[0]["shid"];
		showDialog("审核", 700, 480, url);
	} else {
		showDialog("批量审核", 500, 250, "qgzx_kycxkyxmsh.do?method=kyPlsh");
	}
}
// 批量审核
function savePlsh(shzt, shyj) {
	...
	jQuery.post("qgzx_kycxkyxmsh.do?method=kyPlsh&type=save", {
		shzt : shzt,
		id : guid,
		gwids : gwid
	}, function(data) {
	...
	if (rows.length != 1){
		...
		showConfirmDivLayer("您确定要撤消对该记录的审核操作吗？",{"okFun":function(){
			jQuery.post("comm_spl.do?method=cxshnew",{shlc:splcid,shid:shid},function(data){
				...
				if("1" == data["cancelFlg"]){
					jQuery.post("qgzx_kycxkyxmsh.do?method=cancelSh",{xmid:xmid,shzt:shzt},function(result){
