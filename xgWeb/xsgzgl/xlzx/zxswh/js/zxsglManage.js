	...
	if("11527" == jQuery("#xxdm").val()){
		var gridSetting = {
				...
				colList:[
				   ...
				   {label:'擅长领域',name:'sclydm', index: 'sclydm',formatter : getSclymc}
				],
				sortname: "",
			 	sortorder: ""
			};
		...
		var gridSetting = {
			...
			function getSclymc(cellValue, rowObject) {
				var sclymc = '';
				jQuery.ajaxSetup( {
					async : false
				});
				jQuery.post("xlzx_zxs.do?method=getSclymc", {
					sclydm : cellValue
				}, function(data) {
					sclymc = data;
				}, '');
