 ...
 */
function addCy() {
    url = "dzdy_jgdcygl.do?method=add";
    showDialog("增加教工党支部成员", 770, 550, url, {
        close: function () {
...
function updateCy() {
    ...
    var url = "dzdy_jgdcygl.do?method=updateCy&xh=" + rows[0]["xh"];
    var title = "修改成员信息";
...
function delCy() {
    ...
    showConfirmDivLayer("您确定要删除选择的记录吗？", {
        "okFun": function () {
            jQuery.post("dzdy_jgdcygl.do?method=delCy", para, function (data) {
...
function choseJg() {
    ...
    parentsW.jQuery("#bmmc").html(rows[0]['bmmc']);
    parentsW.jQuery("#symc").html(rows[0]['symc']);
    parentsW.jQuery("#lxdh").html(rows[0]['lxdh']);
    parentsW.jQuery("#jtzz").html(rows[0]['jtzz']);
...
function saveCy() {
    var url = "dzdy_jgdcygl.do?method=saveCy";
    ajaxSubFormWithFun("JgdcyglForm", url, function (data) {
        if (data["message"] == "保存成功！") {
            showAlert(data["message"], {}, {
                "clkFun": function () {
...
function updateSaveCy() {
    var url = "dzdy_jgdcygl.do?method=updateSaveCy";
    ajaxSubFormWithFun("JgdcyglForm", url, function (data) {
        if (data["message"] == "保存成功！") {
            showAlert(data["message"], {}, {
                "clkFun": function () {
