	...
	<head>
		...
		function update(url,flag){
			...
			if(checkRow.size() == 1){
				...
				if(flag=="update"&&jQuery.trim(clzt)!="未处理"){
					return alertInfo("该申请已处理，不能再次分配！");
				}
				
				if(flag=="update"&&mycd!=null&&jQuery.trim(mycd)!=""){
					return alertInfo("已有学生评价，不能再次分配！");
				}
				
				url += "&pk="+curr_row.getElementsByTagName('input')[0].value;
				url += "&xh="+curr_row.getElementsByTagName('input')[1].value;
				//showTopWin(url,800,600);
				showDialog('分配报修申请', 800, 510, url);
				return true;
			}else{
				alertInfo('请选择一条要操作的记录！');
				return false;
			}
		}
		...
		function gybxglExportData() {
			...
			var url = "gyglnew_gybxgl.do?method=gybxglExportData&dcclbh=gyglnew_gybxgl_gybxgl_bxfp.do&flag=bxfp";//dcclbh,导出功能编号
			...
			<div class="toolbox">
				...
				<div class="buttonbox">
					<ul>
						...
						<li><a href="#" onclick="update('gyglnew_gybxgl.do?method=gybxglfpUpdate','update');return false;" class="btn_sh">分配 </a></li>
						<!--  
				...
				<table summary="" class="dateline" width="100%">
					...
					<logic:iterate name="rsArrList" id="s" indexId="index">
						<tr onclick="rowOnClick(this);" ondblclick="update('gyglnew_gybxgl.do?method=gybxglfpView','view')" 
							style="cursor:hand" title="<logic:iterate id="v" name="s" offset="7" length="1">报修内容：${v }</logic:iterate>">
							...
							<!--  
							<logic:iterate id="v" name="s" offset="13" length="1">
								<td title="报修内容：<logic:iterate id="vv" name="s" offset="7" length="1">${vv }</logic:iterate>">${v }</td>
							</logic:iterate>
							<!-- 显示信息 -->
							<!-- 
