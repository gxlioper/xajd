...
<head>
    ...
    <script>
        var gridSetting = {
            caption:"",
            pager:"pager",
            url:"gygl_sfqscj_wh.do?method=shList&type=query",
            colList:[
                ...
                {label:'申请类型',name:'sblx',index:'sblx',width:'10%'},
        ...
        function view(sqid){
            var url="gygl_sfqscj_wh.do?method=view&sqid="+sqid;
            showDialog("岗位申请查看", 700, 600, url);
        }
        function sh(){
            ...
            if(rows.length == 0){
                ...
                var url = "gygl_sfqscj_wh.do?method=sh&sqid="+rows[0]["sqid"]+"&xh="+rows[0]["xh"]+"&gwid="+rows[0]["gwid"]+"&shid="+rows[0]["shid"];
                showDialog("学生宿舍空调申请审核",750,600,url);
            } else {
                showDialog("学生宿舍空调申请批量审核",500,300,"gygl_sfqscj_wh.do?method=plsh");
            }
        }
        //撤销审核
        function cancelSh(){
            ...
            if (rows.length != 1){
                ...
                showConfirmDivLayer("您确定要撤消对该记录的审核操作吗？",{"okFun":function(){
                    jQuery.post("comm_spl.do?method=cxshnew",{shlc:splc,shid:shid},function(data){
                        ...
                        if("1" == data["cancelFlg"]){
                            jQuery.post("gygl_sfqscj_wh.do?method=cancelSh",{sqid:sqid,shzt:shzt},function(result){
...
//                jQuery("#dataTable").initGrid(gridSetting2);
        ...
        function savePlsh(shzt, shyj) {
            ...
            jQuery.post("gygl_sfqscj_wh.do?method=plsh&type=save", {
                shzt : shzt,
                splcs : splcs,
                sqids : guid,
                gwids : gwid,
        ...
        function printWord(){
            ...
            var url;
            if(ids.length == 1){
                url = "gygl_sfqscj_wh.do?method=downloadWord&sqid="+ids[0];
            } else if(ids.length > 1){
                url = "gygl_sfqscj_wh.do?method=downloadZip&sqids="+ids.toString();
            } else {
                showAlert("请至少选择一条记录");
                return false;
            }
...
<html:form action="/gygl_sfqscj_wh" method="post">
