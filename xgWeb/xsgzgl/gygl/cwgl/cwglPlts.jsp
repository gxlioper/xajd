	...
	function loadXsxx(){
		...
		function loadXs(){
			...
			if(count==0){//未选中学生
				...
				jQuery('#xhtd').html("当前查询结果中共有<font color='red'>"+num+"</font>个已入住的床位，可执行退宿操作");
				...
				jQuery('#xhtd').html("当前共选中<font color='red'>"+count+"</font>个已入住床位，可执行退宿操作");
		...
		function saveTsxx(){
			...
			if(!check("tsyy-tssj-xn-xq")){
				return showAlert("请将带<font color='red'>*</font>的项目填写完整！");
					...
					//提示确认信息
				confirmInfo("请检查并确认您要退宿的床位信息和学生信息无误！", function(tag){
					if(tag == 'ok'){
						confirmInfo("确认要提交退宿吗？", function(tag){
							if(tag == 'ok'){
								jQuery("#buttonSave").attr({disabled:'disabled'});
