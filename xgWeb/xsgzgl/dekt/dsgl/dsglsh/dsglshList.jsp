...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			caption:"读书管理未审核列表",
			pager:"pager",
			url:"dekt_dsglsh.do?method=dsglShList&type=query",
			colList:[
			   ...
			   {label:'书名',name:'ssm', index: 'ssm',width:'8%'},
...
function view(sqid, xh) {
	showDialog("查看读书信息", 700, 480, "dekt_dsglsh.do?method=view&sqid=" + sqid+ "&xh=" + xh);
}
...
function Sh(){
	...
	if(shzt=="ysh"){
		...
		var url = "dekt_dsglsh.do?method=dsglDgsh&sqid="+rows[0]["sqid"]+'&xh=' + rows[0]["xh"]+'&shid=' +rows[0]["shid"] ;
		showDialog("审核读书信息",700,500,url);
	} else{
		showDialog("批量审核",500,300,"dekt_dsglsh.do?method=dsglPlsh");
	} 
}
	...
	if (rows.length != 1){
		...
		showConfirmDivLayer("您确定要撤消对该记录的审核操作吗？",{"okFun":function(){
			jQuery.post("comm_spl.do?method=cxshnew",{shlc:splc,shid:shid},function(data){
				...
				if("1" == data["cancelFlg"]){
					jQuery.post("dekt_dsglsh.do?method=cancelSh",{sqid:sqid,shzt:shzt},function(result){
...
function savePlsh(shzt,shyj){
	...
	jQuery.post(
		"dekt_dsglsh.do?method=dsglPlsh&type=save",
		{
		 shzt:shzt,
		 id:guid,
		 gwids:gwid,
...
function ExportData() {
	...
	var url = "dekt_dsglsh.do?method=exportData&dcclbh=" + DCCLBH + "&shlx=" + shlx;//dcclbh,导出功能编号
