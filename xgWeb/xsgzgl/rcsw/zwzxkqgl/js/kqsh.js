...
function kqsh() {
	...
	if (shzt == "ysh") {
		...
		var url = "zwzxKqsh.do?method=kqDgsh&sqid=" + rows[0]["sqid"] +  '&shid=' + rows[0]["shid"];
		showDialog(jQuery("#gnmkmc").val(), 800, 500, url);
	} else {
		showDialog("批量审核", 500, 250, "zwzxKqsh.do?method=kqPlsh");
	}
}
// 批量审核
function savePlsh(shzt, shyj) {
	...
	jQuery.post("zwzxKqsh.do?method=kqPlsh&type=save", {
		shzt : shzt,
		splc : rows[0]["splc"],
		id : guid,
		gwids : gwid,
	...
	if (rows.length != 1){
		...
		showConfirmDivLayer("您确定要撤消对该记录的审核操作吗？",{"okFun":function(){
			jQuery.post("comm_spl.do?method=cxshnew",{shlc:splc,shid:shid},function(data){
				...
				if("1" == data["cancelFlg"]){
					jQuery.post("zwzxKqsh.do?method=cancelSh",{sqid:sqid,shzt:shzt},function(result){
...
var DCCLBH = "zwzxKqsh.do";//dcclbh,导出功能编号
...
function khshExportData() {
	...
	var shzt = jQuery("#shzt").val();
	var url = "zwzxKqsh.do?method=exportData&shzt="+shzt+"&dcclbh=" + DCCLBH;//dcclbh,导出功能编号
