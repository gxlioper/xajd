		...
		<script type="text/javascript">
			...
			jQuery(function(){
				gridSetting = {
						caption:"周报待审核",
						pager:"pager",
						url:"rcsw_xsgzzb_xsgzzbshgl.do?method=xsgzzbshManage&type=query",
						colList:[
						   ...
						   {label:jQuery("#xymc").val(),name:'bmdmmc', index: 'bmdmmc',width:'15%',formatter:bmdmmcLink},
						   {label:'填写时间',name:'lrsj', index: 'lrsj',width:'17%'},
			...
			function xsgzzbSh(){
				...
				if(shzt=="ysh"){
					...
					var url = "rcsw_xsgzzb_xsgzzbshgl.do?method=xsgzzbDgsh&sqid="+rows[0]["sqid"]+'&shid=' +rows[0]["shid"]+'&gwid=' +rows[0]["gwid"] ;
					showDialog("周报审核",700,495,url);
				} else{
					showDialog("周报批量审核",500,220,"rcsw_xsgzzb_xsgzzbshgl.do?method=xsgzzbPlsh");
				} 
			}
				...
				if (rows.length != 1){
					...
					showConfirmDivLayer("您确定要撤消对该记录的审核操作吗？",{"okFun":function(){
						jQuery.post("comm_spl.do?method=cxshnew",{shlc:splc,shid:shid},function(data){
							...
							if("1" == data["cancelFlg"]){
								jQuery.post("rcsw_xsgzzb_xsgzzbshgl.do?method=cancelXsgzzbsh",{shlc:splc,shid:shid,sqid:sqid},function(result){
			...
			var DCCLBH = "rcsw_xsgzzb_xsgzzbsh.do";//dcclbh,导出功能编号
			...
			function xsgzzbshExportData() {
				...
				var url = "rcsw_xsgzzb_xsgzzbshgl.do?method=exportData&dcclbh=" + DCCLBH + "&shlx=" + shlx;//dcclbh,导出功能编号
