...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript" src="xsgzgl/rcsw/hcyhkgl/hcccqjtx/js/hcccqjtxManage.js"></script>
		...
		function submitBusi(){
			...
			if (ids.length != 1){
				...
				for(var i=0;i<ids.length;i++){
					...
					if(rows[i]['shzt']!='3'){
						url = "rcsw_hcyhk_hcccqjtxgl.do?method=submitHcccqjtx";
					}else{
						url = "rcsw_hcyhk_hcccqjtxgl.do?method=submitHcccqjtx";
					}
				}
		...
		function cancel(){
			...
			if (ids.length == 0) {
				...
				showConfirmDivLayer("您确定要撤销选择的记录吗？",{"okFun":function(){
					jQuery.post("rcsw_hcyhk_hcccqjtxgl.do?method=cancelHcccqjtx",
						{
						 values:ids.toString(),
						 splcid : rows[0]['splc'] 
						},function(data){
		...
		function hcccqjtxLcinfo(){
			...
			if (ids.length != 1){
				...
				showDialog("火车优惠卡管理审批流程跟踪",480,380,'comm_spl.do?method=lcgz&sqid='+rows[0]['ccqjtxid']+"&splc="+rows[0]['splc']);
			}
		}
		</script>
	</head>
	...
	<body>
		...
		<html:form action="/rcsw_hcyhk_hcccqjtxgl">
			<input type="hidden" name="tableName" id="tableName" value="xg_rcsw_hcyhk_hcccqjtxb"/>
			...
			<div class="toolbox">
				...
				<div class="buttonbox">
					<ul>
						...
						<li>
							<a href="javascript:void(0);" onclick="hcccqjtxLcinfo();return false;" 
