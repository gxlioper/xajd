...
function khsh() {
    ...
    if (rows.length == 1) {
		var url = "xzhcp_zcsh.do?method=dgsh&sqid=" + rows[0]["sqid"] + '&xh='
				+ rows[0]["xh"] + '&shid=' + rows[0]["shid"];
		showDialog("审核", 770, 550, url);
	} else {
		showDialog("批量审核", 500, 250, "xzhcp_zcsh.do?method=plsh");
	}
}
// 批量审核
function savePlsh(shzt, shyj) {
	...
	jQuery.post("xzhcp_zcsh.do?method=savePlsh", {
		shzt : shzt,
		splc : rows[0]["splc"],
		sqids : guid,
		gwids : gwid,
...
function cancelSh(){
	...
	if (rows.length != 1){
		...
		showConfirmDivLayer("您确定要撤消对该记录的审核操作吗？",{"okFun":function(){
			jQuery.post("xzhcp_zcsh.do?method=cxshnew",{shlc:splc,shid:shid},function(data){
				// 判断是否最后一级撤销(1:最后一级撤销成功）
				if("1" == data["cancelFlg"]){
					jQuery.post("xzhcp_zcsh.do?method=cancelSh",{sqid:sqid,shzt:shzt},function(result){
