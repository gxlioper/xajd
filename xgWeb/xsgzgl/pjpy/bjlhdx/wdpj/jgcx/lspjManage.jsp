...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function printDjbg(){
			...
			jQuery("input[name=primarykey_checkVal]:checked").each(function(i){	
				var pkValue = jQuery(this).val();
				xn[i] = (pkValue.split("luojw")[0]);
				xq[i] = (pkValue.split("luojw")[1]);
				xh[i] = (pkValue.split("luojw")[2]);
				jxjName[i] = (pkValue.split("luojw")[3]);	
			})
			...
			if(!flag){
				alertError("您共勾选了"+num+"条记录，但他们的<font color='blue'>评奖项目</font>并不一致，无法批量打印，请您确认^_^||");
				return false;
			}
			...
			if(!flag){
				alertError("您共勾选了"+num+"条记录，但他们的<font color='blue'>评奖时间</font>并不一致，无法批量打印，请您确认^_^||");
				return false;
			}
