...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function expToExcel(){
				...
				var url = "pjpy_tjcx_shmddc.do?method=expShmddc";
				url+= "&str_spgw="+spgw;
				url+= "&str_xmdm="+xmdm;
				...
				var shzt = new Array();//审核状态
					n=0;
				jQuery("a[name=a_name_shzt]").each(function(){
					var id = jQuery(this).attr("id");
					var sh = id.replace("a_id_","");
					if(sh !=null && sh!=""){
						shzt[n] = sh;
						n++;
					}
				...
				if(shzt != null && shzt.length>0){
					url+= "&array_shzt="+shzt.join("!!array!!");
				}
				if(input_mhcx != null && input_mhcx.length>0){
					url+= "&str_mhcxz="+input_mhcx;
				}
				if(mhcx_lx != null && mhcx_lx.length>0){
					url+= "&str_mhcxlx="+mhcx_lx;
				}
	...
	<body  ondrag="return false">
		...
		<html:form action="/general_pjpy" method="post">
			...
			<input type="hidden" name="exp_xydm" value="${userDep }"/>
				
			<!-- 数据导出判断-->
			<!-- 学校用户 -->
			<logic:equal name="userType" value="admin">
				<input type="hidden" name="path" value="XxSearch"/>
			</logic:equal>
			...
			<logic:equal name="userType" value="xy">
				<input type="hidden" name="path" value="XySearch"/>
			</logic:equal>
			
			<!-- 班主任用户 -->
			<logic:equal name="userType" value="BzrSearch">
				<input type="hidden" name="path" value="BzrSearch"/>
			</logic:equal>
			
			<!-- 辅导员用户 -->
			<logic:equal name="userType" value="FdySearch">
				<input type="hidden" name="path" value="FdySearch"/>
			</logic:equal>
			
			<!-- 班主任兼任辅导员用户 -->
			<logic:equal name="userType" value="BzrJfdySearch">
				<input type="hidden" name="path" value="FdySearch"/>
			</logic:equal>
			<!-- 数据导出判断end-->
