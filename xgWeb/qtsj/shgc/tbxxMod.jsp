...
%>
	<body>
		...
		<script type="text/javascript">
			function tbxxMod(){
				var xhV = document.getElementById("xhV").value;
				var ndV = document.getElementById("ndV").value;
				var url = "shgc_tbxxwh.do?method=modTbxx&doType=mod";
		...
		<html:form action="/shgc_tbxxwh.do" method="post">
			<div class="title">
				...
				<input type="hidden" id="disableEle" name="disableEle" value="xm-xb-xy-nj-zy-bj-xz-sfzh-sjhm" />
				<input type="hidden" id="readonlyEle" name="readonlyEle" value="" />
				<input type="hidden" id="getStuInfo" name="getStuInfo" value="xm-xb-xymc-nj-zymc-bjmc-sfzh-sjhm" />
				...
				<fieldset>
					...
					<table width="100%" class="tbstyle">
						...
						<tr>
							...
							<td align="left">							
								...
								<input type="hidden" id="xhV" name="xhV" value="${rs.xh}"/>
							</td>	
							<td align="right">
								<font color="red">*</font>年度：
							</td>
							<td align="left">
								<html:select name="rs" property="nd" style="width:90px" styleId="nd" disabled="true">
									<html:options collection="ndList" property="nd"
										labelProperty="nd" />
								</html:select>
								<input type="hidden" id="ndV" name="ndV" value="${rs.nd}"/>
							</td>						
						</tr>
						<tr>
							<td align="right">
						...
						<tr>
						<td colspan="4">
						<input type="hidden" id="tbxzNum" name="tbxzNum" value="<bean:write name="tbxzNum"/>"/>
						<input type="hidden" id="bxxzNum" name="bxxzNum" value="<bean:write name="bxxzNum"/>"/>
						<table class="tbstyle">
						<%--显示已投的保险险种列表信息--%>
						<%--有投保信息--%>
						<logic:notEmpty name="tbxzList">												
						<logic:iterate id="tbxz" name="tbxzList" offset="0" indexId="index">
						<tr>		
								<td align="right">
								...
								<td align="left" width="160px">
									...
									<input type="hidden" id="bxxzdmV${index}" value="${tbxz.tbxzdm}">
								</td>									
								<td align="right">
									保险凭证号：
								</td>
								<td align="left">
									...
									<input type="hidden" id="bxpzhV${index}" value="${tbxz.bxpzh}">
								</td>								
								<td align="right">
									保费：
								</td>
								...
								<input type="hidden" id="bfV${index}" value="${tbxz.bf}">
								</td>
							</tr>							
						</logic:iterate>
						<script>
							   var num = document.getElementById("tbxzNum").value;
							   for(var i=0;i<parseInt(num); i++){
							    document.getElementById("bxpzh" + i).value = document.getElementById("bxpzhV" + i).value;
							    document.getElementById("bxxzdm" + i).value = document.getElementById("bxxzdmV" + i).value;
							    document.getElementById("bf" + i).value = document.getElementById("bfV" + i).value;
							   }								
							</script>
						<logic:iterate id="bxxz" name="bxxzList" length="${bxxzNum-tbxzNum}" indexId="index">
							<tr>											
								<td align="right">
									<font color="red">*</font>投保险种：
								</td>
								<td align="left" width="160px">
									<select id="bxxzdm${tbxzNum+index}" name="bxxzdm${tbxzNum+index}" style="width:160px" onchange="changeBf(${tbxzNum+index})">	
									<option></option>
									 <logic:iterate id="bxxz" name="bxxzList">
									 	<option value="${bxxz.bxxzdm}">${bxxz.bxxzmc}</option>								 	
									 </logic:iterate>			
								...
								<td align="left">
									<input type="text" id="bxpzh${tbxzNum+index}" name="bxpzh${tbxzNum+index}"/>
								</td>								
								<td align="right">
									保费：
								</td>
								<td align="left">
								<input type="text" id="bf${tbxzNum+index}" name="bf${tbxzNum+index}" onkeyup="value=value.replace(/[^\d]/g,'') " />(元)
								</td>
							</tr>
						</logic:iterate>
						</logic:notEmpty>
					...
					<div class="buttontool" align="center">						
						<button class="button2" onclick="tbxxMod()"
