...
<head>
	...
	<script language="javascript">
		function add(){
			 var url = "/xgxt/xsgbxx.do?method=gzzjAdd&doType=add";
		...
		function update(){
		  if(curr_row==null){
		   ...
		   var tableName = document.getElementById("tableName").value;
		  var url = "/xgxt/xsgbxx.do?method=gzzjAdd&doType=update&pk=";
		...
		function viewxljk(){
			  ...
			  var tableName = document.getElementById("tableName").value;
			  var url = "/xgxt/xsgbxx.do?method=gzzjAdd&titleType=view&pk=";
			  var pk = curr_row.cells[0].getElementsByTagName("input")[0].value;
			  url += pk;
			  url += "&tableName="+tableName;
			  //showOpenWindow(url, 650, 400);
	...
	<body onload="checkuser();">
		<html:form action="/xsgbxx.do" method="post">
			<div class="tab_cur" id="jd">
				<p class="location">
					<em>您的当前位置:</em><a>社会工作  - 学生干部  - 工作总结</a>
				</p>
			</div>
			<input type="hidden" name="zyV" id="zyV" />
			<input type="hidden" name="bjV" id="bjV" />
			<input type="hidden" id="username" name="username" value="<bean:write name="userType" scope="session"/>" />
			<input type="hidden" id="tableName" name="tableName" value="zjlg_gzzjb" />
			<input type="hidden" id="realTable" name="realTable" value="zjlg_gzzjb" />
							 		...
							 		<td colspan="6" >
										...
										<div class="btn">
					             			 <button class="btn_cx" id="search_go" 
												onclick="refreshForm('/xgxt/xsgbxx.do?method=gzzjManage&go=go');">
												查询
											 </button>
