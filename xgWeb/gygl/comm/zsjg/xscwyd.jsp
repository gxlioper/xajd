...
<%--		function showLcxx(lddm){--%>
<%--			--%>
<%--			gyglCwgl.getLdcsList(lddm,function(data){--%>
<%--				for(i=0;i<data.length;i++){--%>
<%--					var qss=data[i].qss;--%>
<%--					var yfps=data[i].yfps;--%>
...
<%--						rzrs="0";--%>
<%--					}--%>
<%--					$("lc_"+lddm+"_"+cs+"_qss").innerHTML="<em class='chamber'>寝室（个）:"+qss+"</em>";--%>
<%--					$("lc_"+lddm+"_"+cs+"_yfps").innerHTML="<em class='chamber'>已分配（个）:"+yfps+"</em>";--%>
<%--					$("lc_"+lddm+"_"+cs+"_rzrs").innerHTML="<em class='chamber'>已入住（人）:"+rzrs+"</em>";--%>
<%----%>
<%--				}					 		 --%>
<%--			})--%>
<%--		}--%>
		...
		function createHid(xh){
			  var ydxsxhArr=document.getElementById("ydxsxhArr");
			  ydxsxhArr.options[$(ydxsxhArr).options.length] = new Option(xh,xh);     
		}
		
		function removeHid(xh){
			  var ydxsxhArr=document.getElementById("ydxsxhArr");
			 
			  for(i=0;i<ydxsxhArr.options.length;i++){
				if(ydxsxhArr.options[i].value==xh){
					$(ydxsxhArr).options[i]=null;
				}                
			 }
		}
		...
		function xsrzcw(qscwxxArr,id,xmid){
			if($("tr_0")){
				...
				var cwhArr=document.getElementsByName('cwhArr');
				...
				for(i=0;i<xhArr.length;i++){
					if(xhArr[i].value==xh){
						var idArr=(xhArr[i].id).split("_");
						var id_str=idArr[0]+"_"+idArr[1]+"_"+idArr[2]+"_"+idArr[3]+"_"+idArr[4];
						var cwid="cwid_"+idArr[1]+"_"+idArr[2]+"_"+idArr[3]+"_"+idArr[4];
						var cwxmid=id_str+"_xmxs";
						var addhtml="<a class='intake' href='#' onclick=\"xsrzcw('"+qscwxxArr+"','"+id+"','"+xmid+"');return false;\">入住</a>";
						$(id_str+"_cz").innerHTML=addhtml;
						$(id_str+"_xh").value="";
						$(id_str+"_xm").value="";
						$(id_str+"_xb").value="";
						$(id_str+"_xydm").value="";
						...
						$(id_str+"_zymc").value="";
						$(id_str+"_nj").value="";
						$(id_str+"_xmxs").innerHTML=cwhArr[i].value+"号床:未分配";
						break;
					}
				}
				...
				if(xb==qsxb){
					if(kfhz=="可以"){
						...
						if(fpdx=="xy" && xymc==qsfpbm){
							...
							$(id+"_xmxs").innerHTML=cwh+"号床:"+xm;
							blog=true;
						}else if(fpdx=="njxy"  && qsfpbm==xymc+"("+nj+")"){
							$(id+"_xh").value=xh;
							$(id+"_xmxs").innerHTML=cwh+"号床:"+xm;
							blog=true;
						}else if(fpdx=="njzy"  && qsfpbm==zymc+"("+nj+")"){
							$(id+"_xh").value=xh;
							$(id+"_xmxs").innerHTML=cwh+"号床:"+xm;
							blog=true;
	...
	<%--			replaceTable(qscwxxArr,xh,id);--%>
		...
		function replaceTable(qsxx,xxxx,xh,id){
			...
			var rowsArr=document.getElementsByName("rowsArr");
		...
		//返回手动分配
		function fhydxx(){
			if (confirm("将要返回宿舍异动管理界面，请确认已经保存了异动信息！")) {
				refreshForm("gygl_zsjg_ydgl.do?go=go");
			}
		}
		...
		function save(){
			var ydxsxhArr=document.getElementById("ydxsxhArr")
			var ydxhArr=new Array();
			 for(i=0;i<ydxsxhArr.options.length;i++){
				ydxhArr[i]=ydxsxhArr.options[i].value;
			 }
			 
			 
			for(i=0;i<ydxhArr.length;i++){
			...
			saveUpdate('/xgxt/gyglZsjg.do?method=xscwyd&doType=save','');
		}
	...
	<body onload="clickTr();">
		...
		<div class="tab_cur">
			<p class="location">
				<em>您的当前位置：</em><a>公寓管理 - 住宿管理 - 宿舍异动</a>
			</p>
		</div>
		...
		<html:form action="/gyglCwgl">
			...
			<select id="ydxsxhArr"  style="display:none"></select>
			<div class="main_box">
				<div class="mid_box">
					<div class="title">
						<p>
				...
				<div class="leftframe04" style="height:500px; overflow:auto; overflow-x:hidden ">
				    ...
				    <div id="fpcwxsxxMain"> 	
						<table summary="" class="datelist nowrap" align="" width="100%" id="xsmd">
									<!-- 表头 -->
									<thead>
									...
									<logic:notEmpty name="fpcwxsList">
										<logic:iterate name="fpcwxsList" id="s" indexId="index">
											<tr onclick="rowOnClick(this);" style="cursor:hand" id="tr_${index}">
												<!-- 显示信息 -->
												<td>
												...
												<td>
													...
													<input type="hidden" id="xymc_${index}" name="xsxymcArr" value="${s.xydm}"/>
													...
													<input type="hidden" id="zymc_${index}" name="xszymcArr" value="${s.zydm}"/>
										...
										<tr>
											<td >
												...
												<div class="btn">
													 ...
													 <button onclick='fhydxx()'>返回分配结果</button>
												</div>
											</td>
										</tr>
									<thead>
