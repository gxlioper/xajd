	...
	<head>
		...
		function showQsxx(lddm,cs){
			...
			if(!$(lddm+"_"+cs+"_cs")){
				...
				var xsArr=new Array();
				//寝室分配列表
				gyglCwgl.getQsxxList(lddm,cs,xsArr,function(data){
					var lddmbf="";
					var csbf="";
					var qshbf="";
					var n=0;
					...
					for(i=0;i<data.length;i++){
						...
						for(j=0;j<cwxxArr.length;j++){
							...
							if("未分配"==fpbm ){
								...
								xxHtml+="<span id='"+cwid+"_cz'><a class='cancel_01'  href='#' onclick=\"xsqxcw('"+qscwxxArr+"','"+cwid+"','"+xmcwid+"');return false;\">取消</a></span>";
							}
							//学号(保存住宿信息 主键)
...
<%--				replaceTable(qscwxxArr,xh,id);--%>
		...
		function save(){
			saveUpdate('/xgxt/gyglCwgl.do?method=xscwfp&doType=save','');
		}
		...
		function fhcwxxtj(){
			if (confirm("将要返回前一页面，请确认已经保存了分配信息！")) {
				refreshForm("gygl_cwgl_sdfp.do?go=go&doType=query");
			}
		}
	...
	<body onload="clickTr();">
		...
		<div class="tab_cur">
			<p class="location">
				<em>您的当前位置：</em><a>公寓管理 - 床位管理 - 床位分配22</a>
			</p>
		</div>
		<!-- 标题 end-->
										...
										<thead>
											<tr>
												...
												<th colspan="4">
													...
													<em class='chamber'
														id='lc_<%=ldxxMap.get("lddm")%>_<%=ldlcMap.get("cs")%>_yfps'>&nbsp;</em>
													<em class='chamber'
														id='lc_<%=ldxxMap.get("lddm")%>_<%=ldlcMap.get("cs")%>_rzrs'>&nbsp;</em>
												</th>
											</tr>
										</thead>
										<!-- 楼层 内容 -->
