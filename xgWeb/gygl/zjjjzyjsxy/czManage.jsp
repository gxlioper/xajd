...
<html xmlns="http://www.w3.org/1999/xhtml">
					...
					<table width="100%" border="0">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<html:text  property="cz" style="width: 120px" styleId="cz"/>
								</td>
								<th>
									姓名
								</th>
	...
	<script type="text/javascript">
	       function czManageAdd(){
	            var url = "/xgxt/zjjjzy_Gygl.do?method=czAdd";
	            showTopWin(url,"600","400");
	       }
	       function czManageModi(){
	           if (curr_row == null) {
		          alert("请选要修改的记录！\n单击一行记录即可");
		          return false;
	           } else {
	            var url = "/xgxt/zjjjzy_Gygl.do?method=czModi";
	            url +="&pkValue=";
	            url +=(curr_row.cells[0].getElementsByTagName("input"))[0].value;
	            showTopWin(url,"600","400");
	           }
	       }
	       function czManageView(){	          
	            var url = "/xgxt/zjjjzy_Gygl.do?method=czModi";
	            url +="&pkValue=";
	            url +=(curr_row.cells[0].getElementsByTagName("input"))[0].value;
	            url +="&view=true";
	       ...
	       function czManageDel(){
	           if (curr_row == null) {
		          alert("请选要删除的记录！\n单击一行记录即可");
		          return false;
	           } else {
	              if(confirm("确定要删除该记录？")){
	                 var url = "/xgxt/zjjjzy_Gygl.do?method=czDel";
	                 url +="&pkValue=";
	                 url +=(curr_row.cells[0].getElementsByTagName("input"))[0].value;
	                 refreshForm(url);
	              }else{	 
