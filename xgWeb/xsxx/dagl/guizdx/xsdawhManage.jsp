...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function changeByOne(pk){
			...
			showTopWin("/xgxt/guizdxDagl.do?method=xsdawhUpdate&doType=add&xh="+pk,600,450);
		}
		
		// -------显示------------
		function showView(pk){

			showTopWin("/xgxt/guizdxDagl.do?method=xsdawhUpdate&doType=view&xh="+pk,600,450);
		}
		
		function save(){
			var xsdaInfo=document.getElementsByName("xsdaInfo");
			var blog=false;
			for(i=0;i<xsdaInfo.length;i++){
				if(xsdaInfo[i].checked){
					blog=true;
				}
			}
			...
			refreshForm("/xgxt/guizdxDagl.do?method=xsdawhManage&doType=save");
	...
	<body>
		...
		<html:form action="/guizdxDagl" method="post">
			...
			<logic:iterate  name="lxList" id="lx" indexId="index">
				<input type="hidden" name="dmArr" styleId="dm_${index}" value="${lx.dm}"/>
				<input type="hidden" name="mcArr" styleId="mc_${index}" value="${lx.mc}"/>
			</logic:iterate>
			...
			<div class="toolbox">
				...
				<div class="buttonbox">
					<ul>
						<li>
							<a href="#" onclick="changeXsdaInfo()" class="btn_zj"> 新生档案管理 </a>
						</li>
					</ul>
				</div>
				<div class="searchtab">
								...
								<td colspan="6">
									<div class="btn">
										<button type="button" class="btn_cx" id="search_go"
											onclick="allNotEmpThenGo('/xgxt/guizdx_dagl_xsda.do')">
											查 询
										</button>
										&nbsp;&nbsp;&nbsp;&nbsp;
						...
						<tbody>
							<logic:notEmpty name="rsArrList">
								<logic:iterate name="rsArrList" id="s">
									<tr onclick="rowOnClick(this);" ondblclick="showView('<logic:iterate id="v" name="s" offset="0" length="1">${v}</logic:iterate>')"
										style="cursor:hand">
										<td width="15px">
