...
<%@ include file="/syscommon/pagehead_V4.ini"%>
	...
	<script language="javascript">		
		function commitData(){
			saveData('xjxxdj.do?method=xjxxdjModi&type=save','xh');
		}
			
		function delFj(fileId){
			var pk = "xh";
			var filepath = $(fileId).value;
			var pkValue =  $("xh").value;				
			if (confirm("确认要删除该附件?")) {
				var url="/xgxt/xjxxdj.do?method=deleteFile";
	...
	<body onload="checkWinType();">		
		<html:form action="/xjxxdj.do" method="post" enctype="multipart/form-data" >
			...
			<div class="tab_cur">
				<p class="location">
					<em>您的当前位置:</em><a>${title}-<logic:equal value="view" name="type">查看</logic:equal><logic:notEqual value="view" name="type">修改</logic:notEqual> </a>
				</p>
			</div>
			...
			<table width="100%" class="formlist">
				...
				<tr>
					...
					<td>
						<html:text name='rs' property="save_xh" readonly="true" styleId="xh" maxlength="20"/>
					</td>
					<th>出生地</th>
					<td>
				...
				<tr>					
					<th>报考照片</th>
					<td>
						<logic:present property="bkzp" name="rs">
							<img src="<%=request.getContextPath()%>/xsxx/bjqnzzxy/xszptp.jsp?pkValue=${rs.xh }&filePath=${rs.bkzp}" />
							<br/>					
						</logic:present>
				...
				<tr>					
					<th>入学照片</th>
					<td>
						<logic:present property="rxzp" name="rs">
							<img src="<%=request.getContextPath()%>/xsxx/bjqnzzxy/xszptp.jsp?pkValue=${rs.xh }&filePath=${rs.rxzp}" />
							<br/>					
						</logic:present>
					...
					<th>毕业照片</th>
					<td>
						<logic:present property="byzp" name="rs">
							<img src="<%=request.getContextPath()%>/xsxx/bjqnzzxy/xszptp.jsp?pkValue=${rs.xh }&filePath=${rs.byzp}" />
							<br/>					
						</logic:present>
