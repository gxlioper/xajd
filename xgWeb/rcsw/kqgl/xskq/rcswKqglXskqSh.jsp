...
<html xmlns="http://www.w3.org/1999/xhtml" lang="gb2312">
	...
	<script language="javascript">
	    function doQuery(){
	    	...
	    	allNotEmpThenGo('rcswKqglXskq.do?method=rcswKqglXskqSh&doType=qry');
	    }
		function modi(url){
			if(curr_row != null){
		...
		function shTg(){
			$("save_shzt").value="通过";
			dataBatch('rcswKqglXskq.do?method=rcswKqglXskqSh&doType=modi&shjg=通过');
			
		}
		function shBtg(){
			$("save_shzt").value="不通过";
			dataBatch('rcswKqglXskq.do?method=rcswKqglXskqSh&doType=modi&shjg=不通过');
		}
	</script>
	<body>
		<html:form action="rcswKqglXskq" method="post">
			...
			<input type="hidden" name="realTable" id="realTable" value="RCSW_KQGL_XSKQ" />
			<html:hidden property="save_shzt" value="" styleId="save_shzt" />
			<input type="hidden" id="url" name="url" value="/rcswKqglXskq.do?method=rcswKqglXskqSh" />
			<div class="title">
				<div class="title_img" id="title_m">
					当前所在位置：日常事物 - 考勤管理 - 学生请假审核			
				</div>
			</div>			
			<fieldset>
				<legend>
				...
				<table width="100%" class="tbstyle">
					<thead>
						<tr>
							<td align="left">
								&nbsp;学年：
								<html:select disabled="true" property="xn" styleId="xn">
									<html:options collection="xnList" property="xn"
										labelProperty="xn" />
								</html:select>
								<html:hidden property="queryequals_xn" value="${xn}"/>	
								&nbsp;学期：
								<html:select disabled="true" property="xq" styleId="xq">
									<html:options collection="xqList" property="xqdm"
										labelProperty="xqmc" />
								</html:select>
								<html:hidden property="queryequals_xq" value="${xq}"/>								
								&nbsp;学号：
								<html:text property="querylike_xh" size="15" styleId="xh"></html:text>
								姓名：
								<html:text property="querylike_xm" styleId="xm"></html:text>
								<logic:notEqual name="isGygly" value="true">
								请假类型:
								<html:select property="queryequals_qjlx" styleId="qjlx">
								...
								<logic:equal name="isGygly" value="true">
								请假类型:
								<html:select property="queryequals_qjlx" styleId="qjlx">
									<html:option value="">夜不归宿</html:option>
								</html:select>
								</logic:equal>
							</td>
							<td width="10" align="center" valign="middle" rowspan="3">
									<input type="hidden" name="go" value="a" />
										...
										<button type="button" class="button2" style="height:25px" id="cz"
											onclick="czSearchCond('nj-xn-xq-xh-xm-xy-zy-bj-qjlx-fdysh-xysh-shzt-qjqssj-qjjssj-id');">
											重 置
										</button>
							</td>
						</tr>
						...
						<td>
						&nbsp;请假时间：
						 <html:text property="querygreaterequal_qjqssj" styleId="qjqssj" 
							onclick="return showCalendar('qjqssj','y-mm-dd');" 
							onblur="dateFormatChg(this)" readonly="true" />至
						 <html:text property="querylessequal_qjjssj" styleId="qjjssj" 
							...
							<logic:equal name="isGygly" value="true">
								...
								styleId="fdysh" onchange="initZyList();initBjList();">
								<html:option value=""></html:option>
							...
							<logic:equal name="userType" value="xy">
								...
								styleId="xysh" onchange="initZyList();initBjList();">
								<html:option value=""></html:option>
								<html:option value="不通过" >不通过</html:option>
								<html:option value="通过" >通过</html:option>
								<html:option value="未审核" >未审核</html:option>
							...
							<logic:equal name="userType" value="xx">
								...
								styleId="shzt" onchange="initZyList();initBjList();">
								<html:option value=""></html:option>
								<html:option value="不通过" >不通过</html:option>
								<html:option value="通过" >通过</html:option>
								<html:option value="未审核" >未审核</html:option>
					...
					<table width="99%" id="rsTable" class="tbstyle">
						...
						<logic:iterate name="rs" id="s">
							<tr onclick="rowMoreClick(this,'',event);"
								ondblclick="modi('rcswKqglXskq.do?method=rcswKqglXskqOne&doType=view')" style="cursor:hand">
								<td>
					...
					<TR>
						<TD>
							<jsp:include flush="true"
								page="/sjcz/turnpage.jsp?form=rcswKqglXskqForm"></jsp:include>
						</TD>
					</TR>
					<TR>
						<TD height=3></TD>
			...
			<div class="buttontool" id="btn" style="position: absolute;left:1%;top:100px" width="100%">	
					<button type="button" class="button2" onclick="modi('rcswKqglXskq.do?method=rcswKqglXskqShOne&doType=view')"
						style="width:80px">
						单个审核
					</button>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" class="button2" onclick="shTg()"
						style="width:80px">
						审核通过
					</button>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" class="button2" onclick="shBtg()"
						style="width:80px">
						审核不通过
					</button>
					</div>
