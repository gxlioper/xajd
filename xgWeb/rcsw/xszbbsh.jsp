		...
		<script language="javascript">
			...
			jQuery(function(){
				...
				if(radio.length>1 && ''==shgw){
					...
					jQuery('#search_go').click(function(){
						allNotEmpThenGo('rcsw_xszgl.do?method=xszbbsh&doType=query');
					});
				}
			})
				...
				<logic:equal value="yes" name="writeAble">
					<div class="buttonbox">
						<ul>
							<li>
								<a href="#" id="btn_shtg"
									onclick="showAuditingWindow('primarykey_cbv','rcsw_xszgl.do?method=xszbbshOne&shgw='+$('shgw').value,700,500)"
									class="btn_sh"> 审核 </a>
							</li>
							<li>
								<a href="#" id="btn_shtg"
									onclick="if(isChecked('primarykey_cbv') && confirm('您确定要取消审核所勾选的记录吗？')){refreshForm('rcsw_xszgl.do?method=xszbbsh&doType=qxsh')}"
									...
									<tr>
										...
										<td>
											<logic:iterate id="x" name="s" offset="3" length="1">
												<a
												href="javascript:showTopWin('rcsw_xszgl.do?method=xszbbshOne&doType=view&pkValue=<logic:iterate id="v" name="s" offset="2" length="1">${v }</logic:iterate>',700,500)"
						...
						<tfoot>
							<tr>
								<td colspan="2">
									...
									<div class="btn">
										<button type="button" name="保存" onclick="refreshForm('rcsw_xszgl.do?method=xszbbsh&doType=plsh&shjg='+$('shjg').value)">
											保 存
										</button>
										<button type="button" name="取消" onclick="closeWindown();return false;">
											取 消
