...
function tbkn(url){
	...
	if (RowsStr=="!!splitOne!!"){
		alert("请选择要修改审核结果为特别困难的记录！");
		return false;
	}
	
	if (!confirm("确定要将所选记录的审核结果改为特别困难？")){
		return false;
	}
	document.forms[0].action=url;
    document.forms[0].submit();
...
function kn(url){
	...
	if (RowsStr=="!!splitOne!!"){
		alert("请选择要修改审核结果为困难的记录！");
		return false;
	}
	
	if (!confirm("确定要将所选记录的审核结果改为困难？")){
		return false;
	}
	document.forms[0].action=url;
    document.forms[0].submit();
...
function bkn(url){
	...
	if (RowsStr=="!!splitOne!!"){
		alert("请选择要修改审核结果为不困难的记录！");
		return false;
	}
	
	if (!confirm("确定要将所选记录的审核结果改为不困难？")){
		return false;
	}
	document.forms[0].action=url;
    document.forms[0].submit();
...
function wsh(url){
	...
	if (RowsStr=="!!splitOne!!"){
		alert("请选择要修改审核结果为未审核的记录！");
		return false;
	}
	
	if (!confirm("确定要将所选记录的审核结果改为未审核？")){
		return false;
	}
	document.forms[0].action=url;
    document.forms[0].submit();
...
function dataExport2() {
							...
							<tr>
								<td align="left">
									...
									<html:select name="rs1" property="xydm" style="width:180px"
										styleId="xy"
										onchange="refreshForm('/xgxt/auditing_shnl_kns.do')">
								...
								<td width="10" align="center" valign="middle" rowspan="2">
									...
									<button type="button" class="button2" style="height:40px" id="search_go"
										onclick="allNotEmpThenGo('/xgxt/auditing_shnl_kns.do')">
										查询
									</button>
								</td>
							</tr>
    ...
    <bean:write name="v"/>
     ...
     "
									ondblclick="chkAssisOne('/xgxt/auditing_shnl_kns_one.do','view')">
									<td align="center">
    ...
    <bean:write name="v"/>
     ...
     "
									ondblclick="chkAssisOne('/xgxt/shnl_kns.do','view')">
									<td align="center">
										<logic:iterate id="v" name="s" offset="1" length="1">
											<input type="checkbox" name="pk"
												value="<bean:write name="v"/>">
			...
			<center>
				<div class="buttontool" id="btn"
					...
					<logic:equal name="isQuery" value="is">
						<button type="button" class="button2" onclick="add('/xgxt/shnl_kns.do');"
							style="width:80px">
							增 加
						</button>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<button type="button" class="button2" onclick="modi('/xgxt/shnl_kns.do');"
							style="width:80px">
							修 改
						</button>
							&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<button type="button" class="button2"
						onclick="tbkn('/xgxt/auditing_shnl_kns_one.do?actDo=tbkn');"
						style="width:80px">
						特别困难
					</button>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" class="button2"
						onclick="kn('/xgxt/auditing_shnl_kns_one.do?actDo=kn');"
						style="width:80px">
						困 难
					</button>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" class="button2"
						onclick="bkn('/xgxt/auditing_shnl_kns_one.do?actDo=bkn');"
						style="width:80px">
						不困难
					</button>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" class="button2"
						onclick="wsh('/xgxt/auditing_shnl_kns_one.do?actDo=wsh');"
						style="width:80px">
						未审核
					</button>
					&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<button type="button" class="button2"
						onclick="del('/xgxt/auditing_shnl_kns_one.do?actDo=del');"
						style="width:80px">
						删 除
					</button>
					&nbsp;&nbsp;&nbsp;&nbsp;
