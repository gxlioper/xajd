...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function searchRs(){
				...
				//按钮控制
				var url = "xszzYbbxAjax.do?method=ybbxcxSearch";
				...
				ybbxcxSearch(url,parameter);
			
		
		}
		
		function ybbxcxSearch(url,parameter){
		...
		function showModi(){
			...
			if(len==1){	
				...
				if(xysh!="未审核" && xysh!='退回'){
					alertInfo("勾选记录已有审核操作，无法修改！");
					return false;
				}
					
				var url="xszz_ybbx_sq.do?doType=modi";
		...
		function showView(){
			...
			if(len==1){	
				...
				var url="xszz_ybbx_sq.do?doType=view";
		...
		function deleteYbbx(){
			...
			if(n>0){
				...
				jQuery("[name=div_pkValue]:checked").each(function(i){
					...
					var xysh=jQuery(this).parent().parent().find("td").eq(5).html();
				
					if(xysh!="未审核" && xysh!='退回'){
						alertInfo("勾选记录中存在已审核操作，无法删除！",function(tag){
				...
				if(blog){
					confirmInfo("该操作将会删除医保报销信息，是否确定继续操作？",function(tag){
						...
						if(tag=="ok"){
							...
							var url= "xszzYbbxAjax.do?method=delete";
			...
			<div class="toolbox" id="dgncz">
				...
				<div class="buttonbox">
					<ul>
						...
						<li>
							<a href="#" onclick="showTopWin('xszz_ybbx_sq.do?doType=save&have=add',800,600);return false;" class="btn_zj">增加</a>
						</li>
						<li>
