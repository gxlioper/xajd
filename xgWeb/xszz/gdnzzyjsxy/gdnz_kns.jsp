	...
	<script language="javascript">
		function yz(titName){
			...
			if(jtqk != null){
	       	...
	       	}else if(jtjjqksm != null){
	         	if(jtjjqksm.replace(/[^\u0000-\u00ff]/g, "**").length > 1000){	         
	          		 alert("家庭经济情况说明不能大于500字！");
	          		 return false;
	       		 }
			}
			document.forms[0].action = "/xgxt/gdnz_kns.do?doType=add&titName=" + titName;
			document.forms[0].submit();
		}
		
		function toPrintOut(titName){//输出相应的打印页面
			document.forms[0].action = "/xgxt/gdnz_knsb.do";
			document.forms[0].submit();
		}
...
<body>
	...
	<logic:equal name="sfksq" value="-2">
		...
		<html:form action="gdnz_kns.do" method="post">
			<input type="hidden" id="titName" name="titName"
				value="<bean:write name="titName" scope="request" />">
			<input type="hidden" id="url" name="url" value="/gdnz_kns.do" />
			...
			<input type="hidden" id="kncdpm" name="kncdpm"
