...
function btg(url){
	...
	var userType = document.getElementById("userType").value;
	if ("xx" != userType){
		if (!confirm("已通过学校审核的数据不会被"+jQuery("#xbmc").val()+"用户修改，确定要执行操作？")){
			return false;
		}
	}
	document.forms[0].action=url;
...
function chkAssisOne(url) {
	if (curr_row == null) {
		...
		showTopWin(url,900,700);
		return true;
	}
}

function add(url){
	return showTopWin(url,900,700);
}
...
function dataExport2() {
	...
	<body onload="xyDisabled('xy');">
		...
		<html:form action="/zgmsxy_xszz.do?method=xssqcx" method="post">
			<div class="tab_cur">
				<p class="location">
					<em>您的当前位置:</em><a>
						助学贷款 - 学生申请 - 学生申请查询
			...
			<logic:equal value="yes" name="writeAble">
				...
				<div class="buttonbox">	
					<ul>
						<li><a href="#" class="btn_zj" onclick="add('/xgxt/zgmsxy_xszz.do?method=xssqAdd');">增加</a></li>
						<li><a href="#" class="btn_xg" onclick="modi('/xgxt/zgmsxy_xszz.do?method=xssqMod');">修改</a></li>
						<li><a href="#" class="btn_sc" onclick="del('/xgxt/zgmsxy_xszz.do?method=xssqcx&go=del');">删除</a></li>
				...
				<table width="100%" border="0" class="">
					<tbody>
						<tr>
							<th>年度</th>
							<td>
								<html:select property="nd" style="width:150px" styleId="nd">
							...
							<td>
								<logic:notEqual value="stu" name="userType">
										<html:text property="xh" styleId="xh" style="width:150px;inputtext"
										styleClass="inputtext"></html:text>
								</logic:notEqual>
								<logic:equal value="stu" name="userType">
									<html:text property="xh" styleId="xh" style="width:150px;inputtext"
									styleClass="inputtext" disabled="true"></html:text>
									<logic:equal value="stu" name="userType">
					...
					<tfoot>
						<tr>
							...
							onclick="allNotEmpThenGo('/xgxt/zgmsxy_xszz.do?method=xssqcx&go=go')">
							查 询
							</button>
							...
							<table width="99%" id="rsTable" class="dateline tablenowrap">
								...
								<logic:iterate name="rs" id="s">
									<tr onclick="rowOnClick(this)"
										ondblclick="chkAssisOne('/xgxt/zgmsxy_xszz.do?method=xssqView')"
