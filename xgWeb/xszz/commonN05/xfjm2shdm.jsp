...
function tg(url){
	...
	//if("xy" != userType){
		...
		//dd_html += "减免金额：";		
		//dd_html += "</td><td>";
		//dd_html += "<input type='text' value=\"\" id='xxpzjmje' onkeyup=\"value=value.replace(/[^\\d]/g,'')\" />";
		//dd_html += "</td></tr>";	
		//dd_html += "<tr height='30' bgcolor='EEF4F9'>";
		//dd_html += "<td align='center' colspan='2'>";
		//dd_html += "<button class='button2' onclick='commitSh(\"" + url + "\")'>确定</button>&nbsp;&nbsp;";//szGroup()
...
function commitSh(url){
	var pzjmje = val('xxpzjmje');	
	url += "&pzjmje=";
	url += pzjmje;
	if(pzjmje == "" || pzjmje == null){
		alert('请填写批准减免金额！');
		return false;
	}
	refreshForm(url);
...
function dataExport2() {
	document.forms[0].action = "/xgxt/n05_xszz.do?method=xfjm2Exp";
	...
	function tjjg(){
		url = "n05_xszz.do?method=xfjm2shDeptModel&go=tjjg";
		//判断是否已经提交
		var pk = "bm||dm||zjz||tjxm||tjzt";
		var pkV = 'xy'+val('userDep') + val('xn') + val('xq')+ "xfjm2" + "1";
		qgzxgzkh.checkExists('xszz_com_bmshtjb',pk,pkV,function(data){
			if(data){
				...
				pk = "xn||xq||xydm";
				pkV = val('xn') + val('xq')+val('userDep');
				xszzCommonN05DWR.jlsfqbsh('xfjm2','xn!!xq',pk,pkV,val('shjb'),function(data){
					if(data){
	...
	<body onload="xyDisabled('xy');">
		<html:form action="/n05_xszz.do?method=xfjm2shdm" method="post">
			<div class="title">
			...
			<input type="hidden" id="cbVal" name="cbVal" value="" maxlength=""/>
			<fieldset>
				<legend>
					条件选择
				</legend>
						...
						<tr>
							...
							<td width="10" rowspan="" align="center" valign="middle">
								...
								<button class="button2" style="height:40px" id="search_go"
									onclick="allNotEmpThenGo('/xgxt/n05_xszz.do?method=xfjm2shDeptModel&go=go')">
									查询
								</button>
							</td>
						</tr>
							...
							<table width="99%" id="rsTable" class="tbstyle">
								...
								<logic:iterate name="rs" id="s">
									<tr onclick="rowOnClick(this)"
										ondblclick="chkAssisOne('/xgxt/n05_xszz.do?method=xfjm2shdmOne')"
    ...
    <bean:write name="v"/>
     ...
     ">
										...
										<td>
											<a href="#" onclick="rowOnClick(findParents(this,'TR'));chkAssisOne('/xgxt/n05_xszz.do?method=xfjm2shdmOne&doType=view')"><font color="blue">查看详细</font></a>
										</td>
									</tr>
								</logic:iterate>
							</table>
					...
					<logic:equal value="yes" name="writeAble" scope="request">
	                	<div class="buttontool" align="center" id="btn" style="position:absolute;width:95%;top:100px">
							<button class="button2" onclick="tg('/xgxt/n05_xszz.do?method=xfjm2sh&go=tg');"
								style="width:80px">
								通 过
							</button>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<button class="button2" onclick="btg('/xgxt/n05_xszz.do?method=xfjm2sh&go=btg');"
								style="width:80px">
								不通过
							</button>
							<!--学校用户-->
							...
							&nbsp;&nbsp;&nbsp;&nbsp;
							<button class="button2" onclick="btg('/xgxt/n05_xszz.do?method=xfjm2sh&go=th');"
								style="width:80px">
								退回重审
							</button>
							</logic:notEqual>
