...
<head>
	...
	function batch(yesNo){
		...
		//alert(Rows[0].getElementsByTagName("input")[0].value);
		for (i=0; i<Rows.length; i++){ //连接字符串
   			pkString +=Rows[i].getElementsByTagName("input")[0].value+"!!SplitOneSplit!!";
		}
		if (confirm("确定要批量操作吗？")) {
		    refreshForm('qgzxYrdwGhxssh.do?pkString=' + pkString + '&type=' + yesNo + '&go=go');
		}
	}
	</script>
	<body onload="xyDisabled('xy')">		
		<html:form action="/qgzxHgsqsh" method="post">
			...
			<input type="hidden" id="bjV" value="<bean:write name="bj" scope="request"/>" />
			<div class="tab_cur">
				<p class="location">
				  ...
				  <div class="searchtab">
					<table width="100%" border="0">
				          ...
				          <td colspan="6">
				            <div class="btn">
								...
								<button type="button" id="search_go"
									onclick="allNotEmpThenGo('qgzxYrdwGhXsGwsh.do')">
									查询
								</button>
				            </div>
				          </td>
					  ...
					  <tr>
						<th>申请类型</th>
						<td>
							<select name="bj" id="bj" style="width:120px">
								<option value=""></option>
								<option value="申请">申请</option>
								<option value="换岗">换岗</option>
				...
				<tbody>
			      <logic:iterate name="rs" id="s">
					<tr onclick="rowMoreClick(this,'',event)"
						style="cursor:hand;"
						ondblclick="viewInfo('modi','qgzxYrdwGhxsModi.do')">
						<td>
