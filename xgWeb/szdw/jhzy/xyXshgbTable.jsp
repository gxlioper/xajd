	...
	<script language="javascript">
     ...
     function  yzXy(){
         ...
         if(userType=='xy'){
              ...
              var xymc = $('xymc').innerHTML;
              if(trim(xymc)==trim(bmmc)){
              	return true;
              }else{
                alert('<bean:message key="lable.xsgzyxpzxy" />只能修改本<bean:message key="lable.xsgzyxpzxy" />的相关信息');
              	return false;
              }
         }else{
         	return true;
		...
		<html:form action="/show_classStudentCadre_list" method="post">
			<div class="title">
				<div class="title_img" id="title_m">
						当前位置：思政队伍-班级情况-<bean:message key="lable.xsgzyxpzxy" />学生会干部
				</div>
			</div>
			<logic:empty name="rs">
				<br />
			...
			<logic:notEmpty name="rs">
				...
				<input type="hidden" name="url" property="url" value="/xyXshgb_updata.do?act=xyXshzw" />
				<input type="hidden" name="userType" id = "userType" value="<bean:write name ="userType" scope = "session"/>" />
				<input type="hidden" name="bmmc" id = "bmmc" value="<bean:write name = "bmmc" scope ="session"/>" />
					...
					<tr style="height:22px">
						<td align="right">
							...
							<td align="left">
								...
								<button type="button" onclick="showTopWin('/xgxt/sxjy_stu_info.do?act=xyXshzw&pk=<bean:write name='rs' property="pk" />',750,550);"
				...
				<div class="buttontool" align="center">
					<button type="button" class="button2"
						onclick="if(yzXy()){sxjyCommonSave('/xgxt/xyXshgb_save.do?type=xyXshzw&','bjgb-xh','xyXshzw')}">
						保  存
					</button>
				</div>
			</logic:notEmpty>
