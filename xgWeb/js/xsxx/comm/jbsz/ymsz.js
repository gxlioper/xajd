...
function showSzDiv(){
	...
	viewTempDiv("单行操作","czDiv",300,240);
...
function setXsqBySz(){
	...
	if(zpxs == "是"){
		...
		if(parseInt(zpszhs) > parseInt(szhs)){
	...
	xsqHtml +="<div id=\"xsq"+xsqdm+"_div\">";
	xsqHtml +="<table class=\"formlist\" border=\"0\" align=\"center\" style=\"width: 90%\" id=\"xsq"+xsqdm+"\">";
	xsqHtml +="<thead><tr><th colspan=\"4\"><span><a href=\"#\" onclick=\"hiddenXsq('"+xsqdm+"')\">"+xsqmc+"</a></span>";
	...
	if(zpxs == "是"){
		...
		if(zpwz == "右"){
			...
			for(var i=2;i<=zpszhs;i++){
				...
				xsqHtml +="<input type=\"hidden\" name=\"yyzd"+xsqdm+"\" id=\"xsq"+xsqdm+"_"+i+"_left_text\" value=\"\"/>";
				xsqHtml +="</td>";
			...
			for(var i=2;i<=zpszhs;i++){
				...
				xsqHtml +="<input type=\"hidden\" name=\"yyzd"+xsqdm+"\" id=\"xsq"+xsqdm+"_"+i+"_right_text\" value=\"\"/>";
				xsqHtml +="</td>";
