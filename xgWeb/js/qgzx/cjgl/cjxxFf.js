...
function checkGs(ind,gdgcjbz){
	...
	if(gs<20){
//		showAlertDivLayer("固定岗低于20个工时不能发放酬金！");
...
/*					if(sxzd=="je"){
		...
		if(sxzd=="gs" && jQuery("#xxdm").val() == "10351" && jQuery(xh[i]).parent().parent().find("input[name='sfkns']").val() =='困难生'){
			...
			if(parseFloat(dyje)>parseFloat(cjsx) && jQuery("#xxdm").val() != "11647"){
				if(jQuery("#xxdm").val() != "10704"&&gwxz[i]!="固定岗"){
		...
		if(""!=jQuery(je[i]).val() /*&& "0"!=jQuery(je[i]).val()*/){
			if(jQuery("#xxdm").val() != '10351'){
				...
				if(sxzd=="gs"&&parseFloat(jQuery(gs[i]).val())>parseFloat(jQuery("#sxsz").val())){
					...
					if(jQuery("#xxdm").val() != "10704"&&gwxz[i]!="固定岗"){
						alertInfo("<font color='red'>["+jQuery(xh[i]).val()+"]</font>发放金额不能大于学生每月酬金上限("+jQuery("#sxsz").val()+"元)!");
			     		return false;
					}
				}
			}
