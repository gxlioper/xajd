...
function jssfPlwh(){
	...
	if(num >= 1){
		...
		//showTopWin("/xgxt/szdw_dwwh.do?method=cxJzgxx&zgh="+zgh,830,600); .
		showDialog('教师身份批量维护', 350, 200, "szdw_fdyxx.do?method=jssfPlwh&zghs="+zghs);
		//window.open ('/xgxt/szdw_dwwh.do?method=cxJzgxx&zgh='+zgh);
	}else{
		alertError("请勾选<font color='blue'>记录</font>，再进行操作！");
		return false;
	}
}
...
function savejssfPlwh(){
	if(jQuery("[name='jssf']:checked").length == 0){
		showAlert("请将带<font color='red'>*</font>的必填项填写完整！");
		return false;
	}
