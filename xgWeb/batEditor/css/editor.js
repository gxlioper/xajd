...
function doSubmit() {
	...
	//parent.MyAlert(document.getElementsByTagName("input")[0].value);
...
function PasteWord() {
	...
	if (BrowserInfo.IsIE55OrMore) {
		...
		if (confirm("\u6b64\u529f\u80fd\u8981\u6c42IE5.5\u7248\u672c\u4ee5\u4e0a\uff0c\u4f60\u5f53\u524d\u7684\u6d4f\u89c8\u5668\u4e0d\u652f\u6301\uff0c\u662f\u5426\u6309\u5e38\u89c4\u7c98\u8d34\u8fdb\u884c\uff1f")) {
			format("paste");
		}
	}
	eWebEditor.focus();
...
function setMode(NewMode) {
	if (NewMode != sCurrMode) {
		...
		sCurrMode = NewMode;
		disableChildren(document.getElementById("eWebEditor_Toolbar"));
...
function ShowDialog(url, width, height, optValidate) {
	...
	//var arr = window.open(url,"",'toolbar=no,location=no,directories=no,status=yes,menub ar=no,scrollbar=no,resizable=no,copyhistory=yes,width="+w+"px,height="+h+"px,left=-2px, top=-2px,screenX=0px,screenY=0px');
	eWebEditor.focus();
}
...
function Maximize() {
	...
	window.open("dialog/fullscreen.htm?style=" + config.StyleName, "FullScreen" + sLinkFieldName, "toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=150,height=100");
}
