...
public class BzjlsqshAction extends SuperAction {
	// 是否班级评议.1：是，0：否
	 ...
	 * ActionForward 返回类型
	 */
	@SystemAuth(url = "bzjl_sq.do")
	public ActionForward viewJxsqList(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		
		BzjlsqshModel model = (BzjlsqshModel) form;
		BzjlsqshService service = new BzjlsqshService();
		...
		request.setAttribute("path", "bzjl_sq.do");
...
//		String bjdms = request.getParameter("bjdms");
		...
		//得到各申报项目的最后级审核状态及岗位
		BzjlsqshService sqService = new BzjlsqshService();
	...
	public ActionForward toCheckPjsq(ActionMapping mapping, ActionForm form,
		...
		CsszService csszService = new CsszService();
		CsszModel csszModel = csszService.getModel();
		BzjlsqshService sqshService = new BzjlsqshService();
		BzjlsqshService sqService = new BzjlsqshService();
	 ...
	 * ActionForward 返回类型
	 */
	@SystemAuth(url = "pj_jxsq.do",rewritable=ReadWrite.WRITEABLE)
	public ActionForward updateSqb(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		
		BzjlsqshModel sqshForm = (BzjlsqshModel) form;
		BzjlsqshService service = new BzjlsqshService();
		
		BzjlsqshModel model = service.getModel(sqshForm);
...
//			XsxxService xsxxService = new XsxxService();
		...
		if (!StringUtil.isNull(values)) {
			...
			for (int i = 0; i < dataList.size(); i++) {
				...
				if(conditionsAllOk && bjdxmList.size() > 0){
					...
					Map<String,Object> xmsqInfoMap = sqshService.getXmsqInfoList(xh);
	...
	public ActionForward cancle(ActionMapping mapping, ActionForm form,
		...
		if(result){
			...
			if(SFBJPY_Y){
				BzjlsqshModel modelNew = service.getModel(model);
...
//				xsxxService.getXsjbxxMore(model.getXh());
	...
	public ActionForward editSqb(ActionMapping mapping, ActionForm form,
		...
		BdpzService bdpzService = new BdpzService();
		request.setAttribute("jbxxList",bdpzService.getJbxxpz(JXSQ));
		request.setAttribute("path", "xpj_sqsh.do?method=editSqb");
	...
	public ActionForward selectPjxm(ActionMapping mapping, ActionForm form,
		...
		Map<String,Object> resultMap = service.getXmsqInfoList(sqshForm.getXh());
...
//			XsxxService xsxxService = new XsxxService();
	...
	public ActionForward getXmje(ActionMapping mapping, ActionForm form,
			...
			throws Exception {		
		BzjlsqshModel sqshForm = (BzjlsqshModel) form;
		String xmje = new BzjlsqshService().getXmje(sqshForm.getXmdm());
...
//		}
	...
	public ActionForward pjxmShtj(ActionMapping mapping, ActionForm form,
		...
		List<HashMap<String,String>> pjxmList = xmwhService.getOthers(" ");
	...
	public ActionForward exportData(ActionMapping mapping, ActionForm form,
		...
		// 查询
		List<HashMap<String, String>> resultList = service.getAudingListSingle(model,
				user);// 查询出所有记录，不分页
...
//		request.setAttribute("model", model);
	...
	public ActionForward savePlsh(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		BzjlsqshModel myForm = (BzjlsqshModel) form;
		BzjlsqshService service = new BzjlsqshService();
		User user = getUser(request);
	 ...
	 * @throws
	 */
	public ActionForward viewZdtj(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		if (QUERY.equalsIgnoreCase(model.getType())){
			...
			List<HashMap<String,String>> resultList = service.getHjtjb (model);
			JSONArray dataList = JSONArray.fromObject(resultList);
			response.getWriter().print(dataList);
			return null;
		}
		
		return mapping.findForward("viewZdtj");
	}
...
//		HashMap<String,String> xsjbxx = xsxxService.getXsjbxxMore(model.getXh());
	...
	private HashMap<String, String> checkRs(BzjlsqshModel sqshForm) throws Exception {
		// 参数设置信息
	...
	public ActionForward saveHjxx(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws Exception{
		BzjlsqshModel model = (BzjlsqshModel) form;
		boolean rs = new BzjlsqshService().saveHjxx(model);
		JSONObject jsonRs = getJsonMessage(rs+"");
		if(rs){
			CsszModel cssz = new CsszService().getModel();
			List<HashMap<String,String>> rsList = new BzjlsqshService().getHjxxList(model.getXh(), cssz.getXn(),cssz.getXq());
	...
	public ActionForward loadHjxx(ActionMapping mapping, ActionForm form,
		...
		CsszService csszService = new CsszService();
		CsszModel csszModel = csszService.getModel();
		request.setAttribute("hjjgList",new BzjlsqshService().getHjxxList(request.getParameter("xh"), csszModel.getXn(),csszModel.getXq()));
