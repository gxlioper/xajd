...
public class ZtbhJgDao extends SuperDAOImpl<ZtbhJgForm> {
    ...
    protected void setTableInfo() {
        ...
        super.setKey("jgid");
        super.setTableName("xg_sxzzjy_ztbh_ztbhjgb");
    }
    ...
    public List<HashMap<String, String>> getPageList(ZtbhJgForm t, User user) throws Exception {
        ...
        sql.append(" from xg_sxzzjy_ztbh_ztbhjgb a ");
...
//        sql.append(" LEFT JOIN (SELECT bjdm,WM_CONCAT(t2.XM) fdyxm,WM_CONCAT(t2.lxdh) lxdh FROM FDYBJB t1 LEFT JOIN FDYXXB t2 ON t1.ZGH = t2.ZGH  GROUP BY BJDM) f ON a.BJDM = f.BJDM ");
        sql.append(" left join (select jgid,replace(wm_concat(t1.bjmc),';',',') bjmc from XG_ZTBH_BJXXB t left join BKS_BJDM t1 on t.bjdm = t1.bjdm group by jgid ) g on a.jgid = g.jgid ");
    ...
    public boolean doDeleteJg(String[] strings) throws Exception {
        ...
        sql.append(" delete from xg_sxzzjy_ztbh_ztbhjgb  where lcywid in(");
    ...
    public List<HashMap<String,String>> getBjList(ZtbhJgForm t, User user) throws Exception {
        ...
        sql.append(" select * from ( ");
        sql.append(" select b.*,a.bjrs,c.fdyxm,c.lxdh,e.symc ");
        sql.append(" from (select bjdm,count(1) bjrs from view_xsbfxx where (sfzx='在校' or sfzx is null) group by bjdm ");
        sql.append(" ) a ");
        sql.append(" left join view_njxyzybj_all b on a.bjdm=b.bjdm ");
    ...
    public List<HashMap<String,String>> getDCList(ZtbhJgForm t, User user) throws Exception {
        ...
        String[] inputV = SearchService.getTjInput(t.getSearchModel());
        StringBuilder sql = new StringBuilder(" select * from (select (case a.sfsfx when  '0' then '否' when  '1' then '是' else '否' end)as sfsfxmc , a.*,v.xm as hdfzrxm ,b.bjmc from xg_sxzzjy_ztbh_ztbhjgb a left join view_njxyzybj_all b on a.bjdm = b.bjdm  left join view_xsjbxx v on a.hdfzr = v.xh) where 1=1 ");
        sql.append(searchTjByUser);
        sql.append(searchTj);
    ...
    public HashMap<String,String> getBjxx(String bjdm){
        StringBuilder sql = new StringBuilder();
        sql.append(" select * from ( ");
        sql.append(" select b.*,a.bjrs,c.fdyxm,e.symc from ( ");
        sql.append(" select bjdm,count(1) bjrs from view_xsbfxx where (sfzx='在校' or sfzx is null) group by bjdm ");
        sql.append(" ) a ");
        sql.append(" left join view_njxyzybj_all b on a.bjdm=b.bjdm ");
        sql.append(" LEFT JOIN (SELECT bjdm,WM_CONCAT(t2.XM) fdyxm FROM FDYBJB t1 LEFT JOIN FDYXXB t2 ON t1.ZGH = t2.ZGH  GROUP BY BJDM) c ON a.BJDM = c.BJDM ");
        sql.append(" left join XG_XTWH_SYBJGLB d on a.bjdm = d.bjdm ");
        sql.append(" left join XG_XTWH_SYDMB e on d.sydm = e.sydm ");
        sql.append(" ) t1 where 1=1 ");
        sql.append(" and bjdm = ? ");
    ...
    public boolean saveBj(ZtbhJgForm model) throws Exception {
        StringBuilder sql = new StringBuilder();
        sql.append("delete from XG_ZTBH_BJXXB where jgid = ?");
        dao.runUpdate(sql.toString(),new String[]{model.getJgid()});
        sql = new StringBuilder();
        sql.append("insert all");
        List<CjbJxxForm> bjs = model.getBjxxs();
        for(CjbJxxForm j : bjs){
            sql.append(" into XG_ZTBH_BJXXB(jgid,bjdm,ydrs,sdrs,qqrs) values('"+model.getJgid()+"','"+j.getBjdm()+"','"+j.getYdrs()+"','"+j.getSdrs()+"','"+j.getQqrs()+"') ");
        }
    ...
    public boolean delBj(String[] jgids) throws Exception {
        StringBuilder sql = new StringBuilder();
        sql.append("delete from XG_ZTBH_BJXXB where (");
    ...
    public List<HashMap<String,String>> getBjxxByJgid(String jgid){
        StringBuilder sql = new StringBuilder();
        sql.append("select a.*,c.fdyxm,e.symc,f.bjmc from XG_ZTBH_BJXXB a ");
        sql.append(" LEFT JOIN (SELECT bjdm,WM_CONCAT(t2.XM) fdyxm FROM FDYBJB t1 LEFT JOIN FDYXXB t2 ON t1.ZGH = t2.ZGH  GROUP BY BJDM) c ON a.BJDM = c.BJDM ");
        sql.append(" left join XG_XTWH_SYBJGLB d on a.bjdm = d.bjdm ");
        sql.append(" left join XG_XTWH_SYDMB e on d.sydm = e.sydm ");
