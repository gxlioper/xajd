...
public class JgcxDao extends SuperDAOImpl<JgcxForm> {
	...
	public List<HashMap<String, String>> getPageList(JgcxForm t, User user)
		...
		String xsxxbTable = " view_xsbfxx "; // 导出时，把xsxxb换成view_xsxxb
		...
		StringBuilder sfknMcSql = new StringBuilder(); // 是否困难 人数统计
		StringBuilder sfknMc0Sql = new StringBuilder(); // 是否困难 人数统计【0】
		String bknIndex = ""; // 【不困难】在集合中的位置
		String bknMc = ""; // 【不困难】名称
		...
		StringBuilder pddjMcSql = new StringBuilder(); // 评定等级名称
		StringBuilder kndjMcSql = new StringBuilder(); // 困难等级 人数统计
		StringBuilder kndjMc0Sql = new StringBuilder(); // 困难等级 人数统计【0】
		...
		// =============== 评议小组信息 > =============
		// =============== 评议结果 < =============
		sql.append(" decode(t7.shzt,'1', '同意申请', '0', '不同意申请','') bjpyjgshzt,t7.pyhsj bjpyjgpyhsj,t7.pyhdd bjpyjgpyhdd,t7.rdly bjpyjgrdly, ");
		// =============== 评议结果 > =============
		// =============== 统计数据 < =============
...
//		sql.append(" decode(t5.tjzt,'1','已提交','未提交') tjztmc,t5.tjzt, ");
	...
	public HashMap<String,String> getBjpyTjdc(KnsrdbjpyForm knsrdbjpyModel){
		...
		sql.append(" a.pyyxbl then '不困难' else (case when b.ylzd20 / a.sumnum > a.pyyxbl then '一般困难'");
		sql.append("  else (case  when b.ylzd21 / a.sumnum > a.pyyxbl then '特别困难' else ' ' end) end)");
        ...
        sql.append("a.sqr, a.xn, a.xq from (select case when a.ylzd1 = '不困难' then 1  else 0 end ylzd10temp,  case");
        sql.append(" when a.ylzd1 = '困难' then 1 else 0 end ylzd11temp, case when a.ylzd2 = '一般困难' then");
        sql.append(" 1 else  0 end ylzd20temp,  case when a.ylzd2 = '特别困难' then 1 else 0 end ylzd21temp, a.sqr,");
