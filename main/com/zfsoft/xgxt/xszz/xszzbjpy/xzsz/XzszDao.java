...
public class XzszDao extends SuperDAOImpl<XzszForm> {
	...
	protected void setTableInfo() {
		super.setKey("guid");
		super.setTableName("xg_xszz_new_xszz_bjpyxz");
	}
	...
	public List<HashMap<String, String>> getPageList(XzszForm t, User user)
			throws Exception {
		String xnxq = Base.currXn + "" + (StringUtil.isNull(Base.currXq) ? "on" : Base.currXq);
		//生成高级查询相关条件、条件值 
...
//		sql.append(" select a.bjrs,case when d.bjdm is null then 'false' else 'true' end sfksbjpy,nvl(c.xzrs,'0') xzrs,nvl(c.tjzt,'0') tjzt,decode(c.tjzt,'1','已提交','未提交') tjztmc,b.*,c.xms,c.xsdbxm from ( ");
		...
		// ============ 是否开始资助班级评议 < =================
//		sql.append(" left join ( ");
//		sql.append(" select distinct b.bjdm from xg_xszz_new_xszz_bjpyxzpy a ");
//		sql.append(" left join xsxxb b on a.bjpyr=b.xh ");
//		sql.append(" where a.tjzt='1' and a.xn||a.xq='"+xnxq+"' ");
//		sql.append(" ) d on a.bjdm=d.bjdm ");
	...
	public List<HashMap<String, String>> getAddXzszList(XzszForm t, User user) throws Exception {
		...
		sql.append(" and not exists (select 1 from xg_xszz_new_xszz_bjpyxz b where a.bjdm=b.bjdm and a.xh=b.xh) ");
		sql.append(" ) a ");
		sql.append(" left join view_njxyzybj_all b on a.bjdm=b.bjdm ");
	...
	public List<HashMap<String, String>> getUpdateXzszList(XzszForm t, User user) throws Exception {
		...
		sql.append(" select a.xh,a.xm,a.bjdm,b.guid,b.sfxsdb,decode(b.sfxsdb,'1','是','否') sfxsdbmc from xg_xszz_new_xszz_bjpyxz b left join view_xsbfxx a on a.xh=b.xh where b.bjdm =? ");
		sql.append(" ) a ");
		sql.append(" left join view_njxyzybj_all b on a.bjdm=b.bjdm ");
		sql.append(" ) t1 where 1=1 ");
		sql.append(searchTj);
	...
	public boolean sftjNotAll(String bjdm) throws Exception {
		StringBuilder sql = new StringBuilder();
		sql.append(" update xg_xszz_new_xszz_bjpyxz ");
		sql.append(" set ");
		sql.append(" tjr = null, ");
	...
	public boolean delBjpyxzpy(String[] guids) throws Exception {
		...
		String sql = " delete from xg_xszz_new_xszz_bjpyxzpy where xn||xq='"+xnxq+"' and bjpyr=(select xh from xg_xszz_new_xszz_bjpyxz where guid = ?) ";
