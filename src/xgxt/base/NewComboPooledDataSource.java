 ...
 */
public final class NewComboPooledDataSource extends AbstractPoolBackedDataSource implements PooledDataSource, Serializable, Referenceable
{
    final static MLogger logger = MLog.getLogger( NewComboPooledDataSource.class );
                ...
                if ( "connectionPoolDataSource".equals( propName ) )
                {
                    if (val instanceof WrapperConnectionPoolDataSource)
                    {
                        ...
                        if (! (nested instanceof DriverManagerDataSource) )
                            throw new PropertyVetoException("NewComboPooledDataSource requires that its unpooled DataSource " +
                                            " be set at all times, and that it be a" +
                                            " com.mchange.v2.c3p0.DriverManagerDataSource. Bad: " + nested, evt);
                    }
                    else
                        throw new PropertyVetoException("NewComboPooledDataSource requires that its ConnectionPoolDataSource " +
                                        " be set at all times, and that it be a" +
   ...
   final static JavaBeanReferenceMaker referenceMaker = new JavaBeanReferenceMaker();
	...
	synchronized ( this )
		...
		synchronized ( wcpds )
			...
			synchronized( dmds )
			    {
				//System.err.println("NewComboPooledDataSource.getReference()!!!!");
