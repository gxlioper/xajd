...
public class JxjService {
		...
		if ("qry".equalsIgnoreCase(type)) {
			...
			if (UserTypePd.isFdy(isFdy)) {
				return " (case when (select tjzt from PJPY_BMSHTJB b where b.bm='bj' and " +
						"a.bjdm=b.dm and a.xn||a.xq=b.zjz and b.tjxm='jxj')='1' or a.xysh='通过' or a.xxsh='通过'" +
						" then 'disabled' else '' end) dis,(select tjr from pjpy_bmshtjb b " +
						"where b.tjxm='jxj' and b.bm='bj' and b.zjz=a.xn||a.xq and a.bjdm=b.dm and rownum=1) " +
						"提交人,(select tjsj from pjpy_bmshtjb b where b.tjxm='jxj' and b.bm='bj' " +
	...
	public boolean updateJxjplshjg(String[] pkValue, String jg,
		...
		if (pkValue == null) {
			...
			for (int i=0;i<pkValue.length;i++) {
				StringBuilder sql = new StringBuilder("update xsjxjb set ");
				sql.append(sh);
				sql.append("='");
				sql.append(jg);
				sql.append("' where xh||jxjdm||xn||xq = '");
	...
	public boolean updateJxjdgshjg(String pkValue, String sh, String yj,
		...
		sql.append("fdysh".equalsIgnoreCase(shzd) ? "fdyyj" : ("xysh"
				.equalsIgnoreCase(shzd) ? "xyshyj" : "xxshyj"));
		sql.append("=? where xh||jxjdm||xn||xq=?");
	...
	public List<HashMap<String, String>> queryXyJxjshTitle(String userType) {
		...
		String[] cn = new String[] {
				"pk", "dis", "行号", "学年", "学期", Base.YXPZXY_KEY, "班级", "奖学金", "获奖总人数", "获奖总金额",
				"提交人", "提交时间", Base.YXPZXY_KEY+"审核", "学校审核" };
		if (UserTypePd.isXx(userType)) {
			en = new String[] {
					"pk","dis", "r", "xn", "xqmc", "xymc", "bjrs", "je", "xxsh" };
			cn = new String[] {
					"pk","dis", "行号", "学年", "学期", Base.YXPZXY_KEY, "获奖总人数", "获奖总金额", "学校审核"  };
		}
		DAO mydao = DAO.getInstance();
		return mydao.arrayToList(en, cn);
	}
	...
	public boolean updateXyjxjplshjg(String jg, String yj, String[] pkValue,
		...
		if (pkValue == null) {
			...
			jg = "tg".equalsIgnoreCase(jg) ? "通过"
					: ("btg".equalsIgnoreCase(jg) ? "不通过" : "未审核");		
			String sql = "update xsjxjb a set " + shzd + "='";
			sql += jg;
			sql += "',";
			sql += shzd + "yj='";
			sql += yj;
			...
			if (UserTypePd.isXy(userType)) {
				sql += "' where a.fdysh='通过' and exists (select 1 from view_czxx_xsjxjb b where xn||xq||bjdm||jxjdm in ('";
			} else {
				sql += "' where a.xysh='通过' and exists (select 1 from view_czxx_xsjxjb b where xn||xq||xydm in ('";
			}
			...
			sql += ") and a.xn=b.xn and a.xq=b.xq and a.xh=b.xh and a.jxjdm=b.jxjdm)";
	...
	public boolean xyjxjFhcs(String jg, String yj, String[] pkValue,
		...
		if (result) {
			if (UserTypePd.isXy(userType)) {
				StringBuilder sql = new StringBuilder("delete from pjpy_bmshtjb a where a.bm='bj' and a.tjxm='jxj' and exists (select 1 from view_czxx_xsjxjb b where xn||xq||bjdm||jxjdm in ('");
				StringBuilder addSql = new StringBuilder("insert into pjpy_bmshtjb(dm,bm,zj,zjz,tjxm,tjzt,tjr,tjsj) select distinct bjdm,'bj','xn||xq',xn||xq,'jxj','2','");
				addSql.append(userName);
				addSql.append("',to_char(sysdate,'yyyy-mm-dd') from view_czxx_xsjxjb where xn||xq||bjdm||jxjdm in ('");
				for (String s : pkValue) {
					sql.append(s);
					sql.append("','");
					addSql.append(s);
				...
				StringBuilder sql = new StringBuilder("delete from pjpy_bmshtjb a where a.bm='xy' and a.tjxm='jxj' and exists (select 1 from view_czxx_xsjxjb b where xn||xq||xydm in ('");
				StringBuilder addSql = new StringBuilder("insert into pjpy_bmshtjb(dm,bm,zj,zjz,tjxm,tjzt,tjr,tjsj) select distinct xydm,'xy','xn||xq',xn||xq,'jxj','2','");
				addSql.append(userName);
				addSql.append("',to_char(sysdate,'yyyy-mm-dd') from view_czxx_xsjxjb where xn||xq||xydm in ('");
				for (String s : pkValue) {
					sql.append(s);
					sql.append("','");
					addSql.append(s);
...
//	打印报表
	public HashMap<String, String> getPrintMap(String pkValue, String table) {
		return dao.getPrintMap(pkValue, table);
	}
	
	public HashMap<String, String> queryStuZhcjMap(HashMap<String, String> map) {
		if (StringUtils.isNull(map.get("xh")) || StringUtils.isNull(map.get("xn")) || StringUtils.isNull(map.get("xq"))) {
			System.out.println("查询综合成绩时学号,学年,学期不能为空!");
			return null;
		}
		return dao.queryStuZhcjMap(map);
	}
}
