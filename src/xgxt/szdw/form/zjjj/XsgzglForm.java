package xgxt.szdw.form.zjjj;

import xgxt.szdw.form.common.CommonForm;

public class XsgzglForm extends CommonForm{

	/**
	* <p>Title: 学工管理系统</p>
	* <p>Description: 学生信息管理思政队伍-上海工程-教师评优form类</p>
	* <p>Copyright: Copyright (c) 2008</p>
	* <p>Company: zfsoft</p>
	* @author 鲁宁
	* @version 1.0
	*/
	
	private static final long serialVersionUID = -2442235377795701404L;
	
	private String zgh;
	private String shzt;
	private String xm;
	private String	sdbj;
	private String	xyyj;
	private String	xxyj;
	private String	xxsh;
	private String	zwdm;
	private String	zwmc;
	private String	lxdh;
	private String	bz;
	private String	zlsf;
	private String	fgsw;
	private String	jb;
	private String	bmdm;
	private String gw;
	private String rylb;
	private String zyjsdj;
	private String zyjsdjmc;
	private String rylbmc;
	private String sfzh;
	private String bmmc;
	private String sffp;
	
	public String getSdbj() {
		return sdbj;
	}
	public void setSdbj(String sdbj) {
		this.sdbj = sdbj;
	}
	public String getXxsh() {
		return xxsh;
	}
	public void setXxsh(String xxsh) {
		this.xxsh = xxsh;
	}
	public String getXxyj() {
		return xxyj;
	}
	public void setXxyj(String xxyj) {
		this.xxyj = xxyj;
	}
	public String getXyyj() {
		return xyyj;
	}
	public void setXyyj(String xyyj) {
		this.xyyj = xyyj;
	}
	public String getShzt() {
		return shzt;
	}
	public void setShzt(String shzt) {
		this.shzt = shzt;
	}
	public String getZgh() {
		return zgh;
	}
	public void setZgh(String zgh) {
		this.zgh = zgh;
	}
	public String getXm() {
		return xm;
	}
	public void setXm(String xm) {
		this.xm = xm;
	}
	public String getBz() {
		return bz;
	}
	public void setBz(String bz) {
		this.bz = bz;
	}
	public String getFgsw() {
		return fgsw;
	}
	public void setFgsw(String fgsw) {
		this.fgsw = fgsw;
	}
	public String getLxdh() {
		return lxdh;
	}
	public void setLxdh(String lxdh) {
		this.lxdh = lxdh;
	}
	public String getZwdm() {
		return zwdm;
	}
	public void setZwdm(String zwdm) {
		this.zwdm = zwdm;
	}
	public String getZwmc() {
		return zwmc;
	}
	public void setZwmc(String zwmc) {
		this.zwmc = zwmc;
	}
	public String getZlsf() {
		return zlsf;
	}
	public void setZlsf(String zlsf) {
		this.zlsf = zlsf;
	}
	public String getJb() {
		return jb;
	}
	public void setJb(String jb) {
		this.jb = jb;
	}
	public String getBmdm() {
		return bmdm;
	}
	public void setBmdm(String bmdm) {
		this.bmdm = bmdm;
	}
	public String getBmmc() {
		return bmmc;
	}
	public void setBmmc(String bmmc) {
		this.bmmc = bmmc;
	}
	public String getGw() {
		return gw;
	}
	public void setGw(String gw) {
		this.gw = gw;
	}
	public String getRylb() {
		return rylb;
	}
	public void setRylb(String rylb) {
		this.rylb = rylb;
	}
	public String getRylbmc() {
		return rylbmc;
	}
	public void setRylbmc(String rylbmc) {
		this.rylbmc = rylbmc;
	}
	public String getSfzh() {
		return sfzh;
	}
	public void setSfzh(String sfzh) {
		this.sfzh = sfzh;
	}
	public String getZyjsdj() {
		return zyjsdj;
	}
	public void setZyjsdj(String zyjsdj) {
		this.zyjsdj = zyjsdj;
	}
	public String getZyjsdjmc() {
		return zyjsdjmc;
	}
	public void setZyjsdjmc(String zyjsdjmc) {
		this.zyjsdjmc = zyjsdjmc;
	}
	public String getSffp() {
		return sffp;
	}
	public void setSffp(String sffp) {
		this.sffp = sffp;
	}

}
