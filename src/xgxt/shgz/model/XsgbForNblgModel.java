package xgxt.shgz.model;

import xgxt.shgz.model.common.CommonModel;

public class XsgbForNblgModel  extends CommonModel{
	private String pk;
    private String zzdm;
    private String zzmc;
    private String zyfzr;
    private String fzrlxfs;
    private String zdls;
    private String lslxfs;
    private String xmjb;
    private String xmdm;
    private String zzcys;
    private String bmdm;
    private String xxgblb;
    private String xmmc;
    private String  [] bgbdmList;
    private String   bgbdm;
    private String rzkssj;
    private String rzjssj;
    private String khdj;
    
	public String getKhdj() {
		return khdj;
	}
	public void setKhdj(String khdj) {
		this.khdj = khdj;
	}
	public String getRzjssj() {
		return rzjssj;
	}
	public void setRzjssj(String rzjssj) {
		this.rzjssj = rzjssj;
	}
	public String getRzkssj() {
		return rzkssj;
	}
	public void setRzkssj(String rzkssj) {
		this.rzkssj = rzkssj;
	}
	public String getBgbdm() {
		return bgbdm;
	}
	public void setBgbdm(String bgbdm) {
		this.bgbdm = bgbdm;
	}
	public String[] getBgbdmList() {
		return bgbdmList;
	}
	public void setBgbdmList(String[] bgbdmList) {
		this.bgbdmList = bgbdmList;
	}
	public String getBmdm() {
		return bmdm;
	}
	public void setBmdm(String bmdm) {
		this.bmdm = bmdm;
	}
	public String getFzrlxfs() {
		return fzrlxfs;
	}
	public void setFzrlxfs(String fzrlxfs) {
		this.fzrlxfs = fzrlxfs;
	}
	public String getLslxfs() {
		return lslxfs;
	}
	public void setLslxfs(String lslxfs) {
		this.lslxfs = lslxfs;
	}
	public String getPk() {
		return pk;
	}
	public void setPk(String pk) {
		this.pk = pk;
	}
	public String getXmdm() {
		return xmdm;
	}
	public void setXmdm(String xmdm) {
		this.xmdm = xmdm;
	}
	public String getXmjb() {
		return xmjb;
	}
	public void setXmjb(String xmjb) {
		this.xmjb = xmjb;
	}
	public String getXmmc() {
		return xmmc;
	}
	public void setXmmc(String xmmc) {
		this.xmmc = xmmc;
	}
	public String getXxgblb() {
		return xxgblb;
	}
	public void setXxgblb(String xxgblb) {
		this.xxgblb = xxgblb;
	}
	public String getZdls() {
		return zdls;
	}
	public void setZdls(String zdls) {
		this.zdls = zdls;
	}
	public String getZyfzr() {
		return zyfzr;
	}
	public void setZyfzr(String zyfzr) {
		this.zyfzr = zyfzr;
	}
	public String getZzcys() {
		return zzcys;
	}
	public void setZzcys(String zzcys) {
		this.zzcys = zzcys;
	}
	public String getZzdm() {
		return zzdm;
	}
	public void setZzdm(String zzdm) {
		this.zzdm = zzdm;
	}
	public String getZzmc() {
		return zzmc;
	}
	public void setZzmc(String zzmc) {
		this.zzmc = zzmc;
	}
}
