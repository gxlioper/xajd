...
public class TestModifyAction extends BasicAction {	
...
//			userDep = session.getAttribute("userDep").toString();
	...
	private ActionForward getNewsFile(ActionMapping mapping, ActionForm form,
		...
		//--------2010/4/23 luojw -------------
		//String dir = servlet.getServletContext().getRealPath("batEditor/" + Configuration.FILE_UPLOAD_DIR);
...
//		userType = session.getAttribute("userType").toString();
		...
		if (fpfs.equalsIgnoreCase("zy")) {
			...
			"人数调整" };
			sql = "select xn||nd||nj||fpbm||a.jxjdm 主键,rownum 行号,nj,zymc bmmc,jxjmc,cprs,jsrs,rstz from xyjxjfpb a,"
				+ "jxjdmb b,(select distinct zydm,zymc from view_njxyzybj) c where xn=? and nd=? and bmdm=? and "
				+ "a.jxjdm=b.jxjdm and a.fpbm=c.zydm and a.jxjdm like ? and nj like ? and zydm like ? order by jxjmc,nj,zymc";
			...
			"人数调整" };
			sql = "select xn||nd||nj||fpbm||a.jxjdm 主键,rownum 行号,nj,bjmc bmmc,jxjmc,cprs,jsrs,rstz from xyjxjfpb a,"
				+ "jxjdmb b,(select distinct bjdm,bjmc from view_njxyzybj) c where xn=? and nd=? and bmdm=?"
				+ " and a.jxjdm=b.jxjdm and a.fpbm=c.bjdm and a.jxjdm like ? and nj like ? and bjdm like ? order by jxjmc,nj,bjmc";
