...
public class PjpyZhcpDAO extends CommDAO {
	...
	public List<HashMap<String, String>> getZcjsSql(PjpyGeneralForm model,
		...
		for (int i = 0; i < bldmList.size(); i++) {
			...
			for (int j = 0; j < xjsxmList.size(); j++) {
				...
				//上海戏剧学院 综测总分超过100按100算。
				HashMap<String, String> shxjxyZhcpb = new HashMap<String, String>();
				shxjxyZhcpb.put("sql", getShxjxyZhcpb(zcxn, zcxq, zcnd));
				shxjxyZhcpb.put("xmjb", zcjsMap.get("xmjb"));
				zcjsList.add(shxjxyZhcpb);
			}
	...
	/**
	 * @描述:上海戏剧学院 综测总分超过100按100算。
	 * @作者：zhangjw
	 * @日期：2013-5-15 下午02:26:10
	 ...
	 * @throws
	 */
	private String getShxjxyZhcpb(String zcxn,String zcxq,String zcnd){
		StringBuffer sql = new StringBuffer();
		sql.append("update xg_pjpy_zhcpb b set b.zd1 = (case when to_number(b.zd1)>100 then 100 else to_number(b.zd1) end) where b.xn = '");
		sql.append(zcxn);
		sql.append("' and b.xq ='");
		sql.append(zcxq);
		sql.append("' and b.nd ='");
