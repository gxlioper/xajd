...
public class TjcxTjmdhzService extends CommService implements
	...
	public List<HashMap<String, String>> getTjmdhzTop(TjcxTjmdhzModel model,
		...
		// 学院推荐名单汇总
		en = new String[] { "pjdj", "xm", "xh","bjrs", "dycj",
				"zcf","zcfbjpm","zdcj" };
		cn = new String[] { "评奖等级",  "姓名","学号", "班级<人数>", "德育成绩", "综测成绩",
				"综测排名","最低成绩" };
	...
	public void expTjmdhz(PjpyGeneralForm myForm,TjcxTjmdhzModel model,User user, OutputStream os)
		...
		title.append(xn);
		title.append("学年");
		title.append("学院家庭经济困难学生奖学金推荐名单汇总表");
		...
		ex.printToOneCell_multy(ws, title.toString(), 0, 0, 8, true,
		...
		ws.addCell(new Label(6, 1, "制表时间：    年   月   日", wcf2));
		// 填充表头
		...
		ws.addCell(new Label(4, 2, "班级（人数）", wcf2));
		ws.addCell(new Label(5, 2, "品德行为测评成绩", wcf2));
		ws.addCell(new Label(6, 2, "学年综合测评成绩", wcf2));
		ws.addCell(new Label(7, 2, "学年综合测评成绩班级排名", wcf2));
		ws.addCell(new Label(8, 2, "必修、限选课最低分", wcf2));
