...
public class PjpyZhcpDAO extends xsgzgl.pjpy.general.zhcp.PjpyZhcpDAO {
	...
	public List<Object> getZcByZqList(String xh) {
		...
		top[zcxmByZqList.size() ] = "综测等级";
		colList[zcxmByZqList.size() ] = "zd25";

		top[zcxmByZqList.size()+1] = "综测分班级排名";
	...
	public boolean zsfJs(PjpyGeneralForm model, User user)
		...
		sql.append(" when to_number(zd1) < 60 then '不及格' when to_number(zd1) >= 60 and to_number(zd1) < 70 then '及格' ");
		sql.append("  when to_number(zd1) >= 70 and to_number(zd1) < 80 and zcfbl >80 then '及格' ");
		sql.append(" when to_number(zd1) >= 70 and to_number(zd1) < 80 and zcfbl <= 80 then '良' ");
		sql.append(" when to_number(zd1) >= 80 and to_number(zcfbl) > 80 then '良' ");
		sql.append(" when to_number(zd1) >= 80 and to_number(zcfbl) <= 80 then '优'  end zcf from xg_view_pjpy_cpzpmbl ");
	...
	public List<String[]> getZcList(String xh) {
		...
		// ---------------动态获取显示字段 begin---------------------
		String[] colList = new String[] { "xn", "xqmc", "zd1","zd25" };// 基础字段
