 ...
 * </p>
 * <p>
 * Description: 评奖评优_苏州工业园区_我的评奖(学生)_评奖项目_语言表达_DAO类
 * </p>
 * <p>
...
public class YybdDAO extends CommDAO {
	 ...
	 * @author 伟大的骆
	 */
	public Boolean delYybdSqf(PjpyStuForm model, String[] pkValue, User user) {

		boolean flag = true;

		// 主键
		String pk = "xn||xq||xh||yybdnr||xthdrq";
		// 操作表
		String tableName = "szyq_yybdjzb";
	 ...
	 * @author 伟大的骆
	 */
	public Boolean saveYybdSqf(PjpyStuForm model, List<String[]> params,
			User user) {
		...
		sql.append(tableName);
		sql.append("(xn,xq,xh,yybdnr,xthdrq,sqf)");
		sql.append("values(?,?,?,?,?,?)");
	...
	public List<HashMap<String, String>> getYybdList(PjpyStuForm model) {
		...
		sql.append(" select a.id,case when length(a.yybdnr) > 6 then ");
		sql.append(" substr(a.yybdnr, 0, 6) || '...'  ");
		sql.append("  else a.yybdnr end yybdnr, a.yybdnr allyybdnr, a.xthdrq, ");
		...
		sql.append(" from (select id,xn,xq,xh,yybdnr,xthdrq,sqf, ");
		sql.append(" nvl(bzrshf, 0) bzrshf,bzrsh,nvl(xyshf, 0) xyshf, ");
		sql.append(" xysh,nvl(xxshf, 0) xxshf,xxsh from szyq_yybdjzb a ");
		sql.append(" where xn = ? ");
		sql.append(" and xq = ? ");
		sql.append(" and xh = ?) a ");
		...
		List<HashMap<String, String>> list = dao.getList(sql.toString(),
				new String[] { xn, xq, xh }, new String[] { "id", "yybdnr",
						"allyybdnr", "xthdrq",  "sqf",
	...
	public ArrayList<String[]> getYybdList(PjpyStuForm model, User user)
		...
		tableSql.append("from szyq_yybdjzb b ");
		...
		tableSql.append("where c.xmlx = 'szyq_yybdjzb' ");
		...
		if ("stu".equalsIgnoreCase(yhlx) || "bz".equalsIgnoreCase(yhlx)) {
			...
			tableSql.append("where xmlx = 'szyq_yybdjzb' ");
	 ...
	 * @throws Exception
	 */
	public Boolean editYybdInfo(PjpyStuForm model, User user) throws Exception {

		YybdModel yybdModel = model.getYybdModel();
		// 申请ID
		String id = yybdModel.getId();
		// 语言表达内容
		String yybdnr = yybdModel.getYybdnr()[0];
		// 日期
		String xthdrq = yybdModel.getXthdrq()[0];
		// 申请分
		String sqf = yybdModel.getSqf()[0];

		StringBuilder sql = new StringBuilder();
		sql.append("update szyq_yybdjzb set ");
		sql.append("yybdnr = ? ");
		sql.append(",xthdrq = ? ");
		sql.append(",sqf = ? ");
		sql.append("where id = ? ");
		...
		boolean flag = dao.runUpdate(sql.toString(), new String[] { yybdnr,
				xthdrq, sqf, id });
