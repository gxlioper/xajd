 ...
 * @since 2012.7.23
 */
public class QgzxCjglDAO extends SuperDAOImpl<QgzxCjglForm> {
	 ...
	 * @throws Exception
	 */
	public ArrayList<String[]> getCjxxList(QgzxCjglForm myForm) throws Exception {
		// 高级查询MODEL
		SearchModel searchModel = myForm.getSearchModel();
		// 用户对象
		User user = myForm.getUser();
		String[] colList = new String[] { "pkValue", "r", "xn","yrdwmc", "ffny", "yffrs", "ffrs","ffje", "tjzt" };
		if("12309".equals(Base.xxdm)){
			  colList = new String[] { "pkValue", "r", "xn","yrdwmc", "ffny", "yffrs", "ffrs","jfhbje","zcje","ffje","tjzt" };
		}
		if("xq".equals(QgCommUtilf.getQgzq())){
			if("12309".equals(Base.xxdm)){
				  colList = new String[] { "pkValue", "r", "xn","xqmc","yrdwmc", "ffny", "yffrs", "ffrs","jfhbje","zcje","ffje","tjzt" };
			}else {
				  colList = new String[] { "pkValue", "r", "xn","xqmc","yrdwmc", "ffny", "yffrs", "ffrs","ffje", "tjzt" };
			}
		}
		// 高级查询条件
		...
		sql.append("select a.*, rownum r from (select a.xn || '!!@@!!' || a.yrdwdm || '!!@@!!' || a.ffny || '!!@@!!' || nvl(a.xq,'none') pkValue," +
				...
				"(select count(1) from view_xg_qgzx_xsgwxxb c where a.xn = c.xn and a.yrdwdm = c.yrdwdm and c.zgzt = 'zg' and (a.xq is null or a.xq = c.xq)) yffrs," +
				"(select count(1) from (select a.yf,b.xn,b.yrdwdm,a.xq from xg_qgzx_jcffb a left join xg_qgzx_gwxxb b on a.gwdm = b.gwdm) d" +
				" where a.xn = d.xn and a.yrdwdm = d.yrdwdm and a.ffny = d.yf and (a.xq is null or a.xq = d.xq)) ffrs,(select sum(je)from " +
				"(select a.yf,a.je,b.xn,b.yrdwdm,a.xq from xg_qgzx_jcffb a left join xg_qgzx_gwxxb b on a.gwdm = b.gwdm) e " +
	...
	public List<HashMap<String, String>> getGwdm(QgzxCjglForm model) {
		...
		sql.append(" select gwdm,gwmc from xg_qgzx_gwxxb t ");
		sql.append(" left join xg_qgzx_yrdwdmb t1 on t.yrdwid = t1.id ");
		sql.append(" where t1.xydm = ?");
		/*if ("12309".equals(Base.xxdm)||"10052".equals(Base.xxdm)) {
	 ...
	 * @throws Exception
	 */
	public ArrayList<String[]> getGwryList(QgzxCjglForm model,String query) throws Exception {
		...
		if("10338".equals(Base.xxdm)){
			...
			sql.append("a where 1=1 "+query);
			colList = new String[]{"r","xh","gwdm","xm","gwmc","gwxzmc","zgztmc","sgsj","gs","je","bz","gwcjsx","khdj","xn","xq"};
			if("10351".equals(Base.xxdm)){
				colList = new String[]{"r","xh","gwdm","xm","gwmc","gwxzmc","zgztmc","sgsj","gs","je","bz","gwcjsx","khdj","sfkns","xn","xq"};
			}
		}
	...
	public HashMap<String,String> getFfzt(QgzxCjglForm model) throws SQLException {
	 ...
	 * @return
	 */
	public boolean isOver(QgzxCjglForm model, double zje) {
		String sql=null;
	 ...
	 * @return
	 */
	public HashMap<String, String> getGwxxMap(QgzxCjglForm model) {
	 ...
	 * @return
	 */
	public String getGwffny(QgzxCjglForm model){
	 ...
	 * @throws Exception
	 */
	public boolean bcGwffZt(QgzxCjglForm model) throws Exception {
		boolean flag = true;
	...
	/**
	 *  重载：批量提交之前删除结果表中存在的数据.
	 *
	 * @author <a href="www.gavinshow.com">GavinShow[1426]</a>
	 * @date 2018-05-09 18:47
	 * @param gwxn
	 * @param yrbm
	 * @param ffny
	 * @param gwxq
	 ...
	 * @throw
	 */
	public boolean delCjff(String[] gwxn, String yrbm, String ffny, String[] gwxq) throws Exception {

		List<String> params = new ArrayList<String>();
		StringBuffer sql = new StringBuffer();

		Set<String> set = new HashSet<String>();
		set.addAll(Arrays.asList(gwxn));
		sql.append(" delete from xg_qgzx_cjff where xn in ( ");

		int k = 0;
		for(String gwxnv:set){
			sql.append(" ?");
			params.add(gwxnv);
			if(k!=set.size()-1){
				sql.append(",");
		...
		sql.append(") and yrbm = ? and ffsj = ? and sjbs = '1' ");
		params.add(yrbm);
		params.add(ffny);
		if(gwxq != null && gwxq.length > 0){
			sql.append(" and xq in (");
			for(int i=0;i<gwxq.length;i++){
				sql.append(" ?");
				params.add(gwxq[i]);
				if(i!=gwxq.length-1){
					sql.append(",");
	...
	public boolean tjCjff(String[] params) throws Exception {
		...
		sql.append(" from (select a.*, b.gwmc, b.yrdwdm from xg_qgzx_jcffb a, xg_qgzx_gwxxb b  where a.gwdm = b.gwdm) a	");
	 ...
	 * @return
	 */
	public List<HashMap<String, String>> getCjmxList(QgzxCjglForm model) {
		StringBuilder sql=new StringBuilder();
	 ...
	 * @return
	 */
	public String yzCjffxx(QgzxCjglForm model) {
	 ...
	 * @return
	 */
	public String getStringToSplit(QgzxCjglForm model) {
	 ...
	 * 获取学生酬金发放信息
	 */
	public QgzxCjglForm getCjffXx(GzkhKhjgForm model)throws Exception{
		StringBuffer sql = new StringBuffer();
	...
	public HashMap<String,String> getYxInfo(QgzxCjglForm model) {
		StringBuffer sql = new StringBuffer();
		sql.append(" select count(0) zrs from XG_QGZX_XSGWXXB a ");
		sql.append(" left join VIEW_XG_QGZX_GWXXB b on b.gwdm=a.gwdm ");
		sql.append(" where b.sfyxgw='是' and b.yrdwdm=? and a.zgzt='zg' ");
		if(!StringUtils.isNull(model.getXn())){
			sql.append(" and b.xn='"+model.getXn()+"'");
		}
		if(!StringUtils.isNull(model.getXq())){
			sql.append(" and b.xq='"+model.getXq()+"'");
		}
		String zrs = dao.getOneRs(sql.toString(), new String[]{model.getYrbm()},"zrs");
		sql = new StringBuffer();
		sql.append(" select count(0) yxrs from ");
		sql.append("(select b.yrdwdm,b.gwmc,a.* from XG_QGZX_JCFFB a  ");
		sql.append(" left join XG_QGZX_GWXXB b on b.GWDM=a.gwdm) t ");
		sql.append("where t.yrdwdm=? and t.bz='优秀'");
		if(!StringUtils.isNull(model.getXn())){
			sql.append(" and t.xn='"+model.getXn()+"'");
		}
		if(!StringUtils.isNull(model.getXq())){
			sql.append(" and t.xq='"+model.getXq()+"'");
		}
		if(!StringUtils.isNull(model.getFfny())){
			sql.append(" and t.yf='"+model.getFfny()+"'");
		}
		String yxrs = dao.getOneRs(sql.toString(), new String[]{model.getYrbm()},"yxrs");
	...
	@Override
	public List<HashMap<String, String>> getPageList(QgzxCjglForm t) throws Exception {
		// TODO 自动生成方法存根
		return null;
	}
	...
	@Override
	public List<HashMap<String, String>> getPageList(QgzxCjglForm t, User user) throws Exception {
		// TODO 自动生成方法存根
		return null;
	}
	...
	protected void setTableInfo() {
		super.setClass(QgzxCjglForm.class);
		super.setKey("wbh");
